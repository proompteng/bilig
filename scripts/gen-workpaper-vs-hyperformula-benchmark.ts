...
if (isCheckMode) {
  if (!existsSync(outputPath)) {
    throw new Error(
      `WorkPaper competitive benchmark artifact is missing. Run: bun scripts/gen-workpaper-vs-hyperformula-benchmark.ts`,
    );
  }
  ...
  const expectedShape = normalizeArtifactShape({
    ...
    results: [
      comparableShape(
        "build-from-sheets",
        ["cols", "materializedCells", "rows"],
        ["dimensions", "terminalValue"],
      ),
      comparableShape(
        "single-edit-recalc",
        ["downstreamCount"],
        ["changeCount", "terminalFormula", "terminalValue"],
      ),
      comparableShape("batch-edit-recalc", ["editCount"], ["changeCount", "sampleFormulaValue"]),
      comparableShape(
        "range-read",
        ["cols", "materializedCells", "rows"],
        ["readCols", "readRows", "terminalValue", "topLeftValue"],
      ),
      comparableShape(
        "lookup-no-column-index",
        ["rowCount", "useColumnIndex"],
        ["changeCount", "formulaValue"],
      ),
      comparableShape(
      ...
      leadershipShape(
        ...
        ["formula", "rowCount"],
        ["changeCount", "spillHeight", "spillIsArray", "spillValue"],
      ),
    ],
  });
...
function parseArtifactForShape(serialized: string): CompetitiveBenchmarkArtifactShapeInput {
  const candidate = toRecord(
    ...
    `competitive benchmark artifact ${outputPath}`,
  );
  ...
  return {
    ...
    results: results.map((result, index) => {
      ...
      return {
        ...
        comparable: requireBoolean(
          record.comparable,
          `competitive benchmark result ${index} comparable`,
        ),
        fixture: toRecord(record.fixture, `competitive benchmark result ${index} fixture`),
        comparison:
          record.comparison === undefined
            ? undefined
            : toRecord(record.comparison, `competitive benchmark result ${index} comparison`),
        note:
          record.note === undefined
            ? undefined
            : requireString(record.note, `competitive benchmark result ${index} note`),
        engines: {
          workpaper: toRecord(
...
function normalizeArtifactShape(input: CompetitiveBenchmarkArtifactShapeInput): {
  ...
  return {
    ...
    workloads: input.results.map((result) => ({
      ...
      workpaperStatus: requireString(
        result.engines.workpaper.status,
        `${result.workload} workpaper status`,
      ),
      workpaperVerificationKeys: extractVerificationKeys(result.engines.workpaper),
      hyperformulaStatus: requireString(
        result.engines.hyperformula.status,
        `${result.workload} hyperformula status`,
      ),
...
function readHyperFormulaMetadata(
  root: string,
): CompetitiveBenchmarkArtifact["engines"]["hyperformula"] {
  const fallback = {

...
export function extractClassSurface(sourceText: string, className: string): ClassSurfaceSnapshot {
  ...
  for (const line of lines) {
    ...
    if (depth === 1) {
      const staticAccessorMatch = trimmed.match(
        /^(?:public\s+)?static\s+(?:get|set)\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?:\(\))?\s*(?::|\{)/,
      );
      if (staticAccessorMatch) {
        staticMembers.add(staticAccessorMatch[1]);
      }

      const staticPropertyMatch = trimmed.match(
        /^(?:public\s+)?static(?:\s+readonly)?\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?::|=)/,
      );
      if (staticPropertyMatch) {
        staticMembers.add(staticPropertyMatch[1]);
      }

      const staticMethodMatch = trimmed.match(
      ...
      const instanceAccessorMatch = trimmed.match(
        /^(?:public\s+)?(?:get|set)\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?:\(\))?\s*(?::|\{)/,
      );
      if (instanceAccessorMatch) {
        instanceAccessors.add(instanceAccessorMatch[1]);
      }

      const instanceMethodMatch = trimmed.match(
        /^(?:public\s+)?([A-Za-z_][A-Za-z0-9_]*)(?:<[^>]+>)?\s*\(/,
      );

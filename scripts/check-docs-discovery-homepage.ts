...
export function requireHomepageDiscovery(index: string, siteCss: string): void {
  requireAllIncludes(
    ...
    [
      ...
      '<link rel="stylesheet" href="./assets/site.css?v=2026-05-14-2" />',
      ...
      '<strong>46/46</strong>',
      'Lower mean latency on every comparable workload.',
      'Approximate lookup with duplicates is slower at p95:',
      '<code>1.043x</code>',
      'Benchmark that case before you use the headline.',
      'That every Excel file, browser grid, or customer workbook is faster.',
      'packages/benchmarks/baselines/<wbr />workpaper-vs-hyperformula.json',
      'Mean latency on the shared headless formula suite.',
      'Benchmark notes',
      'Compatibility gaps',
      'Starter issues',
      'benchmark critique',

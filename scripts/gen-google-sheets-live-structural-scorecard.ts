...
function formatJsonForRepo(serializedJson: string): string {
  const tempDir = mkdtempSync(join(tmpdir(), 'google-sheets-live-structural-scorecard-'))
  ...
  if (formatResult.exitCode !== 0) {
    ...
    throw new Error(
      `Unable to format generated Google Sheets live structural scorecard: ${new TextDecoder().decode(formatResult.stderr).trim()}`,
    )
  }

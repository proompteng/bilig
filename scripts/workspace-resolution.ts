...
export const workspaceResolutionJsonPath = join(
  workspaceRootDir,
  "workspace-resolution.generated.json",
);
export const workspaceResolutionTsconfigPath = join(
  workspaceRootDir,
...
function readWorkspaceResolution(rootDir = workspaceRootDir): WorkspaceResolutionMap {
  ...
  if (!existsSync(workspaceResolutionPath)) {
    throw new Error(
      `Missing workspace resolution artifact at ${workspaceResolutionPath}. Run bun scripts/gen-workspace-resolution.ts.`,
    );
  }
...
function createWorkspaceAliasMap(
  ...
  return Object.fromEntries(
    sortedEntries(
      Object.entries(resolution).map(([packageName, entry]) => [
        packageName,
        join(rootDir, entry.sourceEntry),
      ]),
    ),
  );
}
...
export function createViteAliasRecord(
  ...
  const generatedAliases = Object.entries(createWorkspaceAliasMap(rootDir)).map(
    ([find, replacement]) => ({
      find,
      replacement,
    }),
...
export function createTsconfigPaths(
  resolution = readWorkspaceResolution(workspaceRootDir),
): Record<string, string[]> {
  return Object.fromEntries(
    sortedEntries(
      Object.entries(resolution).map(([packageName, entry]) => [

...
function runViteSmoke(
  ...
  const distFiles = listFilesRecursive(distDir).map((filePath) =>
...
function readPackedManifest(tarballPath: string): { name: string; version: string } {
  const parsed = parseJsonRecord(
    ...
    `packed manifest in ${tarballPath}`,
  );
...
function parseNodeSmokeOutput(output: string): {
  ...
  if (
    typeof westUnits !== "number" ||
    typeof eastPrice !== "number" ||
    !isNumberArray(thresholdUnits)
  ) {

...
if (isCheckMode) {
  if (!existsSync(outputPath)) {
    throw new Error(`WorkPaper benchmark baseline is missing. Run: bun scripts/gen-workpaper-benchmark-baseline.ts`)
  }
  ...
  if (JSON.stringify(actualShape) !== JSON.stringify(expectedShape)) {
    throw new Error(`WorkPaper benchmark baseline shape is out of date. Run: bun scripts/gen-workpaper-benchmark-baseline.ts`)
  }
  ...
  return {
    ...
    results: results.map((result, index) => {
      ...
      return {
        ...
        metrics: record.metrics === undefined ? undefined : toRecord(record.metrics, `WorkPaper benchmark baseline metrics ${index}`),
      }
    }),
  }
}

function normalizeBaselineShape(baselineDocument: WorkPaperBenchmarkBaselineShapeInput | WorkPaperBenchmarkBaselineDocument): {
  schemaVersion: 1
  suite: 'workpaper'
  scenarios: Array<{
    detailKeys: string[]

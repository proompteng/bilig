...
const outputPath = join(
  ...
  "benchmarks",
  "baselines",
  "workpaper-vs-hyperformula-expanded.json",
);
  ...
  const expectedShape = normalizeArtifactShape({
    ...
    results: [
      ...
      comparableShape(
        "build-mixed-content",
        ["cols", "rows"],
        ["dimensions", "terminalFormulaValue"],
      ),
      comparableShape(
        "build-many-sheets",
        ["colsPerSheet", "rowsPerSheet", "sheetCount"],
        ["sheetCount", "terminalValue"],
      ),
      ...
      leadershipShape(
        ...
        ["formula", "rowCount"],
        ["spillHeight", "spillIsArray", "spillValue"],
      ),
    ],
  });
...
function readHyperFormulaMetadata(
  localRoot: string,
): ExpandedCompetitiveBenchmarkArtifact["engines"]["hyperformula"] {
  const fallback = {
  ...
  const version =
    typeof pkg.version === "string" && pkg.version.length > 0 ? pkg.version : fallback.version;
...
function isArtifactShapeInput(value: Record<string, unknown>): value is ArtifactShapeInput {
  return (
    ...
    value.suite === "workpaper-vs-hyperformula-expanded" &&

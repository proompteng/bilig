...
function hasNumericCellValue(runtime: WorkbookWorkerRuntime, address: string, value: number): boolean {
  const snapshotValue = runtime.getCell('Sheet1', address).value
  return 'value' in snapshotValue && typeof snapshotValue.value === 'number' && snapshotValue.value === value
}

async function seedWorkerLocalStore(documentId: string, materializedCells: number): Promise<PersistedWorkerSeed> {
  const viewport = {
    sheetName: 'Sheet1',
    rowStart: 0,
    rowEnd: 39,
...
export async function runWorkerVisibleEditBenchmark(materializedCells = 10_000): Promise<WorkerVisibleEditBenchmarkResult> {
  ...
  runtime.subscribeViewportPatches(
    ...
    (bytes) => {
      ...
      if (patch.cells.some((cell) => cell.snapshot.address === 'A1' && cell.displayText === '999')) {
...
export async function runWorkerReconnectCatchUpBenchmark(
  ...
  if (!hasNumericCellValue(runtime, formatAddress(pendingMutationCount - 1, 0), 200_000 + pendingMutationCount - 1)) {
  ...
  if (!hasNumericCellValue(runtime, formatAddress(pendingMutationCount - 1, 1), 300_000 + pendingMutationCount - 1)) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (benchmark === 'warm-start') {
    ...
    console.log(JSON.stringify(await runWorkerReconnectCatchUpBenchmark(materializedCells, pendingMutationCount), null, 2))
  } else {

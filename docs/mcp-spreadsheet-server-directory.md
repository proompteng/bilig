...
Registry API now includes the current `@bilig/headless@0.18.24` package version.
...
Latest checked result on May 17, 2026: npm latest is `@bilig/headless@0.18.24`,
and the official Registry API search now includes Bilig WorkPaper entry version
`0.18.24` with `isLatest: true`. The API also returns historical entries, so

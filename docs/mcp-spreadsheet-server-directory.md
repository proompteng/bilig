...
| ------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Official MCP Registry           | Live through `0.24.0` in public search when requesting enough results                 | <https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.proompteng%2Fbilig-workpaper&limit=100> |
...
Latest checked result on May 19, 2026: npm latest is `@bilig/headless@0.24.0`.
Official Registry search returns Bilig WorkPaper version `0.24.0` when the
...
printf '%s\n' \
  ...
  '{"jsonrpc":"2.0","method":"notifications/initialized"}' \
  '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' |
  npm exec --package @bilig/headless@0.24.0 -- \

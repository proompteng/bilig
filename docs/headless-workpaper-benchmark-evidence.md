...
The overall directional mean-ratio geomean is `0.6367963961549464`. The overall
directional p95-ratio geomean is `0.6531874112281288`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `structural-insert-columns-small`, with a mean ratio of
`2.195659729548466`. The current worst p95 row is `lookup-approximate-sorted-large`,
with a p95 ratio of `2.6263072529213884`. The headless leadership scorecard

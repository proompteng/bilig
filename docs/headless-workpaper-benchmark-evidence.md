...
current checked-in artifact records `43/57` mean-latency wins:
...
The overall directional mean-ratio geomean is `0.7189291803437611`. The overall
directional p95-ratio geomean is `0.7318937181498144`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `structural-append-formula-rows`, with a mean
ratio of `5.664889304744179`. The current worst p95 row is
`structural-append-formula-rows`, with a p95 ratio of `5.174820638071306`. The
headless leadership scorecard currently records `39/57` workloads winning both
...
The `43/57` count is about mean latency: for each winning comparable workload

...
current checked-in artifact records `39/57` mean-latency wins:
...
The overall directional mean-ratio geomean is `0.7472862203775282`. The overall
directional p95-ratio geomean is `0.758380911905582`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `structural-append-formula-rows`, with a mean
ratio of `8.408663250166809`. The current worst p95 row is
`structural-append-formula-rows`, with a p95 ratio of `8.542587980430362`. The
headless leadership scorecard currently records `36/57` workloads winning both
...
The `39/57` count is about mean latency: for each winning comparable workload

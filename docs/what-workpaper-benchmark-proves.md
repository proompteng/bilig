...
`73/94` mean-latency wins on scorecard-eligible comparable workloads. This is a
...
The overall directional mean-ratio geomean is `0.6640533611767504`, and the
overall directional p95-ratio geomean is `0.6755757378251391`. Ratios below
...
scorecard records `69/94` workloads winning both mean and p95. The worst p95
holdout is `structural-insert-columns-large`, where the current WorkPaper-to-HyperFormula
p95 ratio is `2.3108708976954957`. The honest claim is `73/94` mean wins plus an
...
> bilig's WorkPaper benchmark currently records `73/94` mean wins against
...
> command, and an explicit p95 caveat. the claim is `73/94` mean wins for the

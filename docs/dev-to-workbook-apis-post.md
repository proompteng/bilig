...
I keep running into the same spreadsheet automation bug.

The screenshot looks fine. The total changed. The agent says it edited the
right input. But nobody can answer the boring questions that matter in
production:
...
That is the line where a screenshot stops being evidence.
...
It is not an Excel clone. The useful promise is narrower: load or build a
workbook, write inputs, recalculate formulas, read the answer back, and save the
state as JSON.
...
For backend jobs and agent tools, I want the rendered spreadsheet to be the
inspection layer. The contract should be the workbook state.
...
That gives an agent a loggable operation, not just a screenshot.
...
full desktop Excel behavior. For those, use Excel, Google Sheets, or a mature
spreadsheet UI product.

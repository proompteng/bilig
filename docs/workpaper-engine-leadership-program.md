...
| Performance dominance      | Directly comparable benchmark workloads                               | unified benchmark includes `100` scorecard-eligible comparable workloads, public and holdout lanes | HyperFormula has `23` current mean wins in the latest artifact | `WorkPaper` leads `76/100` overall mean workloads and `76/100` mean+p95 workloads; the worst p95 row is `single-formula-edit-recalc` | turn mean wins into larger-margin mean+p95 wins without weakening benchmark gates                    |
...
`performance-leading`: WorkPaper is `76/100` on comparable mean workloads and

...
- that artifact currently shows WorkPaper leading `81/100` scorecard-eligible
  comparable mean workloads and `77/100` mean+p95 workloads
...
| Performance dominance      | Directly comparable benchmark workloads                               | unified benchmark includes `100` scorecard-eligible comparable workloads, public and holdout lanes | HyperFormula has `19` current mean wins in the latest artifact | `WorkPaper` leads `81/100` overall mean workloads and `77/100` mean+p95 workloads; the worst p95 row is `lookup-approximate-duplicates` | turn mean wins into larger-margin mean+p95 wins without weakening benchmark gates                    |
...
`77/100` on mean+p95 workloads. The active program is now about larger margins,
...
- `WorkPaper` now leads most of the expanded competitive engine scorecard:
  ...
  - `77/100` overall scorecard-eligible comparable mean+p95 workloads
  - `62/73` public comparable mean workloads
  - `19/27` holdout comparable mean workloads
  - current artifact generated at `2026-05-17T08:34:05.460Z`

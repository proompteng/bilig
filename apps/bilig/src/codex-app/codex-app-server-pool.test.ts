...
import {
  ...
  CodexAppServerPoolBackpressureError,
...
class FakePoolTransport implements CodexAppServerTransport {
  ...
  async threadResume(input: {
    threadId: string;
  }): Promise<{ id: string; preview: string; turns: [] }> {
...
function createPool(input?: {
  ...
  const pool = new CodexAppServerClientPool({
    codexClientFactory: (options) => {
      ...
      const nextTransport =
        transports.shift() ?? new FakePoolTransport(`transport-${String(capturedOptions.length)}`);
    ...
    ...(input?.maxConcurrentTurnsPerClient !== undefined
      ? { maxConcurrentTurnsPerClient: input.maxConcurrentTurnsPerClient }
      : {}),
    ...(input?.maxQueuedTurnsPerClient !== undefined
      ? { maxQueuedTurnsPerClient: input.maxQueuedTurnsPerClient }
      : {}),
  });
...
function resolvePendingTurn(
  resolveTurn: ((value: CodexTurn) => void) | null,
  value: CodexTurn,
): void {
  if (!resolveTurn) {
...
describe("codex-app-server-pool", () => {
  ...
  it("applies backpressure when the per-client turn queue is full", async () => {
    ...
    try {
      ...
      const firstTurn = pool.turnStart({ threadId: thread.id, prompt: "first" });
      const secondTurn = pool.turnStart({ threadId: thread.id, prompt: "second" });
      await expect(pool.turnStart({ threadId: thread.id, prompt: "third" })).rejects.toBeInstanceOf(
        CodexAppServerPoolBackpressureError,
      );
      resolvePendingTurn(resolveFirstTurn, {
        id: "turn-1",

  ...
  return {
    ...
    hiddenRows: rowEntries
      .filter(
        (entry) =>
          entry.hidden === true &&
          entry.index >= input.range.startRow &&
          entry.index <= input.range.endRow,
      )
      .map((entry) => ({
    ...
    hiddenColumns: columnEntries
      .filter(
        (entry) =>
          entry.hidden === true &&
          entry.index >= input.range.startCol &&
          entry.index <= input.range.endCol,
      )
      .map((entry) => ({
...
export function inspectWorkbookContext(
  runtime: WorkbookRuntime,
  ...
  return JSON.stringify(
    {
      ...
      visibleRange: {
        ...
        cellCount:
          ...
          (visibleRange.endCol - visibleRange.startCol + 1),
      },

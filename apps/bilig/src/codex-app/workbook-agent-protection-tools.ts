...
const deleteRangeProtectionArgsSchema = z
  ...
  .refine(
    (value) => value.id !== undefined || value.range !== undefined || value.selector !== undefined,
    {
...
const hideFormulasArgsSchema = z
  ...
  .refine(
    (value) => (value.sheetName ? 1 : 0) + (value.range ? 1 : 0) + (value.selector ? 1 : 0) === 1,
    {
...
export const workbookAgentProtectionToolSpecs = [
  {
    ...
    description:
      "List workbook sheet and range protection status. Optionally filter by sheet, explicit range, or semantic selector.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Hide formulas on a protected sheet or protected range. For ranges, creates or updates a protection record.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
function resolveSheetName(
  args: { sheetName?: string | undefined },
...
export async function handleWorkbookAgentProtectionToolCall(
  ...
  switch (normalizedTool) {
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.lockCells: {
      ...
      const protection = await context.zeroSyncService.inspectWorkbook(
        context.documentId,
        (runtime) => {
          const resolved = resolveRangeOrSelectorRequest({
            runtime,
      ...
      const protection = await context.zeroSyncService.inspectWorkbook(
        ...
        (runtime) => {
          ...
          const matches = runtime.engine
            .getRangeProtections(normalized.sheetName)
            .filter((entry) => {
              const candidate = normalizeRangeBounds(entry.range);
              return (
                candidate.startAddress === normalized.startAddress &&
                candidate.endAddress === normalized.endAddress
              );
            });
          if (matches.length !== 1) {
            throw new Error(

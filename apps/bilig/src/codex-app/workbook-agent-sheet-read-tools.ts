...
export const workbookAgentSheetReadToolSpecs = [
  ...
  },
  {
    ...
    description:
      "Read sheet-level view state, including used range, freeze panes, filters, and sorts for one sheet.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description:
      "Read the current contiguous populated region around an explicit anchor or the current selection.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.getRowMetadata: {
      ...
      const payload = await context.zeroSyncService.inspectWorkbook(
        ...
        (runtime) => ({
          ...
          entries: filterAxisMetadata(
            runtime.engine.getRowMetadata(args.sheetName),
            args.startIndex,
            args.count,
          ),
        }),
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.getColumnMetadata: {
      ...
      const payload = await context.zeroSyncService.inspectWorkbook(
        ...
        (runtime) => ({
          ...
          entries: filterAxisMetadata(
            runtime.engine.getColumnMetadata(args.sheetName),
            args.startIndex,
            args.count,
          ),
        }),

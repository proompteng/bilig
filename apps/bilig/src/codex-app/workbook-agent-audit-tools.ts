...
export const workbookAgentAuditToolSpecs = [
  ...
  },
  {
    ...
    description: 'Find formulas whose precedent chains traverse hidden rows, optionally scoped to one sheet.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: 'Detect copied-formula runs with outliers in contiguous rows or columns, optionally scoped to one sheet.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: 'Report sheets where metadata extends the effective used range well beyond populated cells.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: 'Verify workbook structural invariants and optionally run a snapshot round-trip stability check.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
...
export async function handleWorkbookAgentAuditToolCall(
  ...
  switch (normalizedTool) {
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.verifyInvariants: {
      ...
      const report = await context.zeroSyncService.inspectWorkbook(context.documentId, (runtime) => verifyWorkbookInvariants(runtime, args))
      return textToolResult(stringifyJson(report))
    }
    default:
      return null

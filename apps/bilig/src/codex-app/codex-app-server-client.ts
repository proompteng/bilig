...
function parseThreadItem(value: unknown): ParsedThreadItem | null {
  ...
  switch (type) {
    ...
    case "dynamicToolCall": {
      ...
      const contentItems =
        contentItemsValue === null
          ? null
          : contentItemsValue.map((entry) => parseToolContentItem(entry));
  ...
  private async startProcess(): Promise<CodexInitializeResponse> {
    ...
    child.once("close", (code, signal) => {
      this.rejectAllPending(
        new Error(
          `Codex app-server exited unexpectedly (${signal ?? "code"}:${String(code ?? "unknown")})`,
        ),
      );
    });
  ...
  private async handleServerRequest(request: ParsedServerRequest): Promise<void> {
    try {
      ...
      this.respondWithError(
        ...
        asError(error, "Dynamic tool call failed").message,
      );
    }
  }

...
import { z } from 'zod'
import { cellRangeRefJsonSchema, workbookSemanticSelectorJsonSchema } from './workbook-agent-selector-tooling.js'
...
export const workbookAgentStructuralToolSpecs = [
  ...
  },
  {
    ...
    description: 'Set frozen top rows and left columns on a sheet. Use rows=0 and cols=0 to clear frozen panes.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
  ...
  },
  {
    ...
    description: 'Hide, unhide, resize, or reset row metadata across a bounded row span on one sheet.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
  ...
  },
  {
    ...
    description: 'Hide, unhide, resize, or reset column metadata across a bounded column span on one sheet.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
...
export function parseWorkbookAgentStructuralToolCommand(request: CodexDynamicToolCallRequest): WorkbookAgentCommand | null {

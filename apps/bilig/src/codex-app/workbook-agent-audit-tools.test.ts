...
describe("workbook agent audit tools", () => {
  it("scans workbook formulas for broken references", async () => {
    ...
    const payload = brokenReferencePayloadSchema.parse(
      await callAuditTool(engine, WORKBOOK_AGENT_TOOL_NAMES.scanBrokenReferences),
    );
  ...
  it("reports used-range bloat from far metadata extents", async () => {
    ...
    const payload = usedRangeBloatPayloadSchema.parse(
      await callAuditTool(engine, WORKBOOK_AGENT_TOOL_NAMES.scanUsedRangeBloat),
    );
  ...
  it("verifies workbook invariants and round-trip stability", async () => {
    ...
    const payload = invariantPayloadSchema.parse(
      await callAuditTool(engine, WORKBOOK_AGENT_TOOL_NAMES.verifyInvariants),
    );

...
export function createWorkbookAgentDeveloperInstructions(): string {
  return [
    ...
    `Use ${WORKBOOK_AGENT_TOOL_NAMES.startWorkflow} with summarizeWorkbook, summarizeCurrentSheet, describeRecentChanges, findFormulaIssues, highlightFormulaIssues, highlightCurrentSheetOutliers, normalizeCurrentSheetHeaders, normalizeCurrentSheetNumberFormats, traceSelectionDependencies, explainSelectionCell, searchWorkbookQuery, createCurrentSheetRollup, createSheet, renameCurrentSheet, hideCurrentRow, hideCurrentColumn, unhideCurrentRow, or unhideCurrentColumn when the request matches those built-in durable workflows and you want the result saved in the thread.`,

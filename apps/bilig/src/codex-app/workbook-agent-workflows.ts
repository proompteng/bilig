...
function summarizeCurrentSheetMarkdown(sheet: ReturnType<typeof summarizeWorkbookStructure>['sheets'][number]): string {
  const lines = [
...
function summarizeRecentChangesMarkdown(changes: Awaited<ReturnType<ZeroSyncService['listWorkbookChanges']>>): string {
...
function summarizeDependencyTraceMarkdown(report: ReturnType<typeof traceWorkbookDependencies>): string {
  const lines = [
    '## Dependency Trace',
    '',
...
function serializeWorkflowCellValue(value: { tag: ValueTag; value?: number | boolean | string; code?: number }): string {
  switch (value.tag) {
    case ValueTag.Empty:
      return '(empty)'

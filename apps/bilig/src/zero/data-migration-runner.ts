...
export async function runPendingZeroDataMigrations(
  ...
  try {
    ...
    await client
      .query(`SELECT pg_advisory_unlock(hashtext($1))`, [DATA_MIGRATION_LOCK_KEY])
      .catch(() => undefined);
    client.release();
  }
}

export function resolveRunDataMigrationsOnBoot(
  env: Record<string, string | undefined> = process.env,
): boolean {
  return parseBooleanEnv(env["BILIG_RUN_DATA_MIGRATIONS_ON_BOOT"]);
}

export function resolveAllowPendingCleanupMigrations(
  env: Record<string, string | undefined> = process.env,
): boolean {

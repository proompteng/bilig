...
export function resolveZeroPublicationName(
...
export async function ensureZeroPublication(
  db: Queryable,
  publicationName = resolveZeroPublicationName(),
): Promise<void> {
  const quotedPublicationName = quoteIdentifier(publicationName);
  if (!(await publicationExists(db, publicationName))) {
    await db.query(
      `CREATE PUBLICATION ${quotedPublicationName} FOR TABLE ${formatQualifiedTableList(ZERO_PUBLICATION_TABLES)}`,
    );
    return;
  }

  const existingTables = await loadPublicationTables(db, publicationName);
  const missingTables = ZERO_PUBLICATION_TABLES.filter(
  ...
  await db.query(
    `ALTER PUBLICATION ${quotedPublicationName} ADD TABLE ${formatQualifiedTableList(missingTables)}`,
  );
}

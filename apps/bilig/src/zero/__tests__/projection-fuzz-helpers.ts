...
function compareProjectionKey(
  left: WorkbookSourceProjection["cells"][number],
  right: WorkbookSourceProjection["cells"][number],
): number {
  return (
    left.sheetName.localeCompare(right.sheetName) ||
    left.rowNum - right.rowNum ||
    left.colNum - right.colNum
  );
}

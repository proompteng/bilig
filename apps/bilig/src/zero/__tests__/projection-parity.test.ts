...
const projectionParityActionArbitrary = fc.oneof<ProjectionParityAction>(
  fc
    .record({ address: addressArbitrary, value: literalInputArbitrary })
    .map((action) => Object.assign({ kind: `value` as const }, action)),
  fc
    .record({ address: addressArbitrary, formula: formulaArbitrary })
    .map((action) => Object.assign({ kind: `formula` as const }, action)),
  fc
    .record({ range: rangeArbitrary, patch: stylePatchArbitrary })
    .map((action) => Object.assign({ kind: `style` as const }, action)),
  fc
    .record({ range: rangeArbitrary, format: numberFormatArbitrary })
    .map((action) => Object.assign({ kind: `format` as const }, action)),
  ...
  fc
    .record({
      settings: fc.record({
        ...
        compatibilityMode: fc.constantFrom<
          WorkbookCalculationSettingsSnapshot["compatibilityMode"]
        >("excel-modern", "odf-1.4"),
      }),
    })
...
describe("projection parity", () => {
  it("should build the same workbook source projection from the engine and exported snapshot", async () => {
    ...
    // Act
    const fromEngine = normalizeProjection(
      buildWorkbookSourceProjectionFromEngine("doc-1", engine, projectionOptions),
    );
    const fromSnapshot = normalizeProjection(
      buildWorkbookSourceProjection("doc-1", engine.exportSnapshot(), projectionOptions),
    );
...
function applyProjectionParityAction(
  engine: SpreadsheetEngine,
  action: ProjectionParityAction,
): void {
  switch (action.kind) {
    case "value":
...
function normalizeProjection(projection: WorkbookSourceProjection): WorkbookSourceProjection {
  return {
    ...
    sheets: [...projection.sheets].toSorted((left, right) =>
      compareTuples([left.sheetId], [right.sheetId]),
    ),
    cells: [...projection.cells].toSorted((left, right) =>
      compareTuples(
        [left.sheetName, left.rowNum, left.colNum],
        [right.sheetName, right.rowNum, right.colNum],
      ),
    ),
    rowMetadata: [...projection.rowMetadata].toSorted((left, right) =>
      compareTuples(
        [left.sheetName, left.startIndex, left.count],
        [right.sheetName, right.startIndex, right.count],
      ),
    ),
    ...
    definedNames: [...projection.definedNames].toSorted((left, right) =>
      compareTuples([left.name], [right.name]),
    ),
    workbookMetadataEntries: [...projection.workbookMetadataEntries].toSorted((left, right) =>
      compareTuples([left.key], [right.key]),
    ),
    ...
    numberFormats: [...projection.numberFormats].toSorted((left, right) =>
      compareTuples([left.id], [right.id]),
    ),
  };
}

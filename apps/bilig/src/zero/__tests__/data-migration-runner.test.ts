...
describe("data migration runner", () => {
  it("runs pending migrations in order and records them in the ledger", async () => {
    ...
    expect(
      client.calls.some((call) => call.text.includes("SELECT pg_advisory_lock(hashtext($1))")),
    ).toBe(true);
    expect(
      client.calls.some((call) => call.text.includes("SELECT pg_advisory_unlock(hashtext($1))")),
    ).toBe(true);
    expect(
      client.calls
        .filter((call) => call.text.includes("INSERT INTO bilig_data_migration"))
        .map((call) => call.values?.[0]),
    ).toEqual(["required-a", "cleanup-b"]);
  ...
  it("fails startup readiness when required migrations are still pending", async () => {
    const client = new FakeMigrationClient([
      (text) =>
        text.includes("FROM bilig_data_migration")
          ? [{ name: "sheet-id-repair" } satisfies QueryResultRow]
          : null,
    ]);

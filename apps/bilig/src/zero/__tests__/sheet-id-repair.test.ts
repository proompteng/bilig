...
import {
  normalizeWorkbookSheetIdAssignments,
  repairWorkbookSheetIds,
  type WorkbookSheetIdRow,
} from "../sheet-id-repair.js";
  ...
  it("repairs duplicate sheet ids and updates dependent tables by sheet name", async () => {
    ...
    expect(
      queryable.calls.some(
        (call) =>
          call.text.includes("UPDATE sheets") &&
          call.values?.[0] === "doc-1" &&
    ...
    expect(queryable.calls.some((call) => call.text.includes("UPDATE workbook_change"))).toBe(
      false,
    );
  });

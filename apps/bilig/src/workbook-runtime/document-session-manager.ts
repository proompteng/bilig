...
import { type InMemoryDocumentPersistence, createInMemoryDocumentPersistence } from '@bilig/storage-server'
...
import { acceptPersistedSnapshotChunk, publishPersistedSnapshot } from './document-snapshot-store.js'
import { createAckFrame, createAppendBatchFrame, createCursorWatermarkFrame, createHeartbeatFrame } from './sync-frame-shared.js'
...
export class DocumentSessionManager {
  ...
  constructor(
    ...
    const browserSessionHost = new WorkbookBrowserSessionHost({
      register: (frame) => joinOwnedBrowserSession(this.persistence, this.ownerId, frame.documentId, frame.sessionId),
      ...
      listMissedFrames: async (documentId, cursorFloor) => {
        ...
        return missed.map((entry) => createAppendBatchFrame(entry.documentId, entry.cursor, entry.batch))
      },
    })
    const syncSessionHost = new WorkbookSyncSessionHost<ProtocolFrame>({
      browserSessionHost,
      hello: async (helloFrame) => {
        await joinOwnedBrowserSession(this.persistence, this.ownerId, helloFrame.documentId, helloFrame.sessionId)
      ...
      appendBatch: async (appendFrame) => {
        const stored = await this.persistence.batches.append(appendFrame.documentId, appendFrame.batch)
        this.broadcast(appendFrame.documentId, createAppendBatchFrame(appendFrame.documentId, stored.cursor, appendFrame.batch))
        return createAckFrame(appendFrame.documentId, appendFrame.batch.id, stored.cursor, stored.receivedAtUnixMs)
      },
      snapshotChunk: async (snapshotFrame) => {
        await this.acceptSnapshotChunk(snapshotFrame)
        return createAckFrame(snapshotFrame.documentId, snapshotFrame.snapshotId, snapshotFrame.cursor)
      },
      heartbeat: async (heartbeatFrame) =>
        createHeartbeatFrame(heartbeatFrame.documentId, await this.persistence.batches.latestCursor(heartbeatFrame.documentId)),
      passthrough: (passthroughFrame) => passthroughFrame,
      unsupported: (unsupportedFrame) =>
        createUnsupportedSyncFrame(unsupportedFrame.documentId, 'UNSUPPORTED_FRAME', unsupportedFrame.kind, 'Unsupported sync frame'),
    })
    this.sessionCore = new WorkbookSessionCore({
      ...
      openWorkbookSession: async (request) => {
        const sessionId = await openPresenceBackedWorkbookSession(this.persistence, request.documentId, request.replicaId)
      ...
      getMetrics: async (request) => ({
        ...
        value: {
          service: 'bilig-app',
          documentSessions: await countPresenceBackedWorkbookSessions(this.persistence, request.sessionId),
        },
      }),
  ...
  private async loadWorkbookFile(request: LoadWorkbookFileRequest, context: AgentFrameContext): Promise<AgentResponse> {
    return loadWorkbookIntoRuntime(
      request,
      context,
      createWorkbookLoadOptions(this.options, {
        registerPreparedSession: async (prepared) => {
          await joinOwnedBrowserSession(this.persistence, this.ownerId, prepared.documentId, prepared.sessionId)
        },
        publishImportedSnapshot: async (documentId, snapshot) => {
          await publishPersistedSnapshot(this.persistence, documentId, snapshot, this.broadcast.bind(this))
        },
      }),
    )
  }

  private async acceptSnapshotChunk(frame: Extract<ProtocolFrame, { kind: 'snapshotChunk' }>): Promise<void> {
    await acceptPersistedSnapshotChunk(this.persistence, this.sessionCore.snapshotAssemblies, frame)
  }

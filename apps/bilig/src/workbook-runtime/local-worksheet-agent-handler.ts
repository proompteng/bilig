...
import type { LocalAgentRangeSubscriptionState, LocalAgentSessionContainer, LocalAgentSessionState } from './local-agent-session-store.js'
import { cellCountForRange, collectChangedAddressesForEvent, getRangeBounds, iterateRange } from './range-subscription-utils.js'
...
export interface LocalWorksheetAgentHandlerContext<SessionState extends LocalWorksheetSessionState = LocalWorksheetSessionState> {
...
export function handleLocalWorksheetAgentRequest<SessionState extends LocalWorksheetSessionState>(
  ...
  switch (request.kind) {
    ...
    case 'subscribeRange': {
      ...
      const watchedAddresses = rangeCellCount <= context.largeRangeSubscriptionThreshold ? iterateRange(request.range) : null
      const unsubscribe = watchedAddresses
        ...
        : session.engine.subscribe((event: EngineEvent) => {
            const changedInRange = collectChangedAddressesForEvent(session.engine, request.range, bounds, event)

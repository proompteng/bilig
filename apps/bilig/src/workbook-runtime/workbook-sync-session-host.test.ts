...
describe('WorkbookSyncSessionHost', () => {
  it('routes sync frames through the shared delegates', async () => {
    ...
    const hello = vi.fn(async () => [{ kind: 'ack', documentId: 'doc-1', batchId: 'h', cursor: 0, acceptedAtUnixMs: 1 }] as ProtocolFrame[])
    const appendBatch = vi.fn(
      async () => [{ kind: 'ack', documentId: 'doc-1', batchId: 'a', cursor: 1, acceptedAtUnixMs: 2 }] as ProtocolFrame[],
    )
    const snapshotChunk = vi.fn(
      async () => [{ kind: 'ack', documentId: 'doc-1', batchId: 's', cursor: 2, acceptedAtUnixMs: 3 }] as ProtocolFrame[],
    )
    const heartbeat = vi.fn(async () => [{ kind: 'heartbeat', documentId: 'doc-1', cursor: 4, sentAtUnixMs: 4 }] as ProtocolFrame[])

...
describe('WorkbookSessionCore', () => {
  it('routes browser, sync, and agent traffic through shared handlers', async () => {
    ...
    const syncSessionHost = new WorkbookSyncSessionHost<ProtocolFrame[]>({
      browserSessionHost,
      hello: async () => [{ kind: 'ack', documentId: 'doc-1', batchId: 'hello', cursor: 0, acceptedAtUnixMs: 1 }],
      appendBatch: async () => [{ kind: 'ack', documentId: 'doc-1', batchId: 'append', cursor: 1, acceptedAtUnixMs: 2 }],
      snapshotChunk: async () => [{ kind: 'ack', documentId: 'doc-1', batchId: 'snapshot', cursor: 2, acceptedAtUnixMs: 3 }],
      heartbeat: async () => [{ kind: 'heartbeat', documentId: 'doc-1', cursor: 4, sentAtUnixMs: 4 }],

...
import type { AgentEvent, AgentFrame, AgentResponse, LoadWorkbookFileRequest } from '@bilig/agent-api'
...
import { flushQueuedLocalAgentEvents, queueLocalAgentEvent, removeQueuedSubscriptionEvents } from './local-agent-event-queue.js'
import {
...
import { handleLocalWorksheetAgentRequest, type LocalWorksheetSessionState } from './local-worksheet-agent-handler.js'
import {
...
import { createAckFrame, createAppendBatchFrame, createHeartbeatFrame } from './sync-frame-shared.js'
...
export class LocalWorkbookSessionManager {
  ...
  constructor(private readonly options: LocalWorkbookSessionManagerOptions = {}) {
    ...
    const syncSessionHost = new WorkbookSyncSessionHost<ProtocolFrame[]>({
      ...
      appendBatch: (appendFrame) => {
        ...
        session.cursor += 1
        const committedFrame = createAppendBatchFrame(appendFrame.documentId, session.cursor, appendFrame.batch)
      ...
      unsupported: (unsupportedFrame) => [
        createUnsupportedSyncFrame(unsupportedFrame.documentId, 'UNSUPPORTED_SYNC_FRAME', unsupportedFrame.kind, 'Unsupported frame'),
      ],
    })
    this.sessionCore = new WorkbookSessionCore({
      ...
      closeWorkbookSession: (request) => {
        ...
        closeLocalAgentSession(session, request.sessionId, (ownedSession, sessionId, subscriptionId) => {
  ...
  private removeAgentSubscription(session: LocalWorkbookSession, sessionId: string, subscriptionId: string): void {
    removeLocalAgentSubscription(session, sessionId, subscriptionId, this.agentSubscriptionOwners, (removedSubscriptionId) => {

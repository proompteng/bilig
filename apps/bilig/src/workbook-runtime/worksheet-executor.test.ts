...
import {
  createHttpWorksheetExecutor,
  createInProcessWorksheetExecutor,
  type HttpWorksheetExecutorOptions,
} from "./worksheet-executor.js";
...
describe("worksheet executor", () => {
  it("posts agent frames to the monolith worksheet endpoint", async () => {
    ...
    const fetchImpl: NonNullable<HttpWorksheetExecutorOptions["fetchImpl"]> = vi.fn(
  ...
  it("forwards agent frames to the in-process worksheet host with configured context", async () => {
    ...
    const handleAgentFrame: DocumentControlService["handleAgentFrame"] = vi.fn(
      (_frame: AgentFrame, _context?: AgentFrameContext) => Effect.succeed(responseFrame),
    );

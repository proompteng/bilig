...
export async function closePresenceBackedWorkbookSession(persistence: InMemoryDocumentPersistence, sessionId: string): Promise<void> {
  await persistence.presence.leave(documentIdFromSessionId(sessionId), sessionId)
}

export async function countPresenceBackedWorkbookSessions(persistence: InMemoryDocumentPersistence, sessionId: string): Promise<number> {

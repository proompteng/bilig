...
export function updateActorFromFrames(actor: ActorLike, frames: ProtocolFrame | ProtocolFrame[]): void {
...
export function wrapTransportPromise<Success>(message: string, run: () => Promise<Success>): Effect.Effect<Success, TransportError> {
...
export function wrapTransportSync<Success>(message: string, run: () => Success): Effect.Effect<Success, TransportError> {
  return Effect.try({
    try: run,
    catch: (cause) =>
      new TransportError({

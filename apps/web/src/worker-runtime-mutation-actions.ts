...
export function recordMutationAttemptInJournal(args: {
  ...
  return updateMutation({
    ...
    id: args.id,
    update: (mutation) =>
      Effect.runSync(recordPendingWorkbookMutationAttempt(mutation, args.attemptedAtUnixMs)),
  });
}
...
export function ackMutationInJournal(args: {
  ...
  return updateMutation({
    ...
    id: args.id,
    update: (mutation) =>
      Effect.runSync(markPendingWorkbookMutationAcked(mutation, args.ackedAtUnixMs)),
  });
}
...
export function failMutationInJournal(args: {
  ...
  return updateMutation({
    ...
    update: (mutation) =>
      Effect.runSync(
        markPendingWorkbookMutationFailed(mutation, args.failureMessage, args.failedAtUnixMs),
      ),
  });
}

export function retryMutationInJournal(args: {
  mutationJournalEntries: readonly PendingWorkbookMutation[];
  id: string;
}): {
  mutationJournalEntries: PendingWorkbookMutation[];

...
export function normalizeProjectedColumnWidth(
  width: number | null,
  ...
  return width === null
    ? null
    : Math.max(minColumnWidth, Math.min(maxColumnWidth, Math.round(width)));
}
...
export class WorkerRuntimeProjectionCommands {
  ...
  async updateRowMetadata(
    ...
    await this.withProjectionMutation((engine) => {
      engine.updateRowMetadata(
        ...
        startRow,
        count,
        normalizeProjectedRowHeight(height),
        hidden,
      );
    });
  }
  ...
  async updateColumnMetadata(
    ...
    const normalizedWidth = normalizeProjectedColumnWidth(
      width,

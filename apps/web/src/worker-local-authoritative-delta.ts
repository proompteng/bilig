...
import { buildWorkbookLocalAuthoritativeBase, buildWorkbookLocalAuthoritativeBaseForSheets } from './worker-local-base.js'
...
function compareSheetIds(
  ...
  return (currentSheetName.get(left) ?? String(left)).localeCompare(currentSheetName.get(right) ?? String(right))
}

function requiresFullAuthoritativeReplace(payloads: readonly WorkbookEventPayload[], engineEvents: readonly EngineEvent[]): boolean {
  return (
    payloads.some(
      (payload) =>
        payload.kind === 'applyBatch' ||
        (payload.kind === 'applyAgentCommandBundle' &&
          payload.bundle.commands.some((command) => command.kind === 'createSheet' || command.kind === 'renameSheet')) ||
...
function collectImpactedSheetIdsFromPayloads(engine: SpreadsheetEngine, payloads: readonly WorkbookEventPayload[]): Set<number> {
  const sheetIds = new Set<number>()
  payloads.forEach((payload) => {
    if (payload.kind === 'updateRowMetadata' || payload.kind === 'updateColumnMetadata' || payload.kind === 'setFreezePane') {
...
function collectImpactedSheetIdsFromEngineEvents(engine: SpreadsheetEngine, engineEvents: readonly EngineEvent[]): Set<number> {
  const sheetIds = new Set<number>()
...
export function buildWorkbookLocalAuthoritativeDelta(input: {
  ...
  const currentSheetOrder = new Map([...engine.workbook.sheetsByName.values()].map((sheet) => [sheet.id, sheet.order]))
  ...
  if (requiresFullAuthoritativeReplace(payloads, engineEvents)) {
    const replacedSheetIds = [
      ...new Set([...previousSheets.map((sheet) => sheet.sheetId), ...currentSheets.map((sheet) => sheet.sheetId)]),

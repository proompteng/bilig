...
export function selectProjectedViewportKeysToEvict(args: {
  ...
  candidates.sort((left, right) => left.accessTick - right.accessTick || left.key.localeCompare(right.key))
  return [...missingSnapshotKeys, ...candidates.map((candidate) => candidate.key)].slice(0, overflow)
}

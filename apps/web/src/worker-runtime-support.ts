...
export function buildAxisPatches(
  ...
  const indices = full
    ? collectAxisIndices(start, end, null)
    : collectAxisIndices(start, end, invalidatedAxes);
...
export function indexAxisEntries(
...
export function collectSheetViewportImpacts(
  engine: WorkerEngine,
  event: EngineEvent,
): Map<string, SheetViewportImpact> | null {
  const changedCellsBySheet =
    event.invalidation !== "full"
      ? collectChangedCellsBySheet(engine, event.changedCellIndices)
      : null;

  ...
  it("preserves patched cell and axis state for monotonic viewport updates", async () => {
    await runProperty({
      ...
      predicate: async (actions) => {
        ...
        expect(store.getHiddenColumns(sheetName)).toEqual(
          Object.fromEntries(expectedHiddenColumns),
        );
...
const viewportActionArbitrary = fc.oneof<ViewportAction>(
  fc
    .record({
      ...
      value: fc.oneof<number | boolean | string | null>(
...
function patchFromAction(action: ViewportAction): ViewportPatch {
  return {
    ...
    columns:
      action.kind === "column"
        ? [{ index: action.index, size: action.size, hidden: action.hidden }]
        : [],
    rows:
      action.kind === "row"
        ? [{ index: action.index, size: action.size, hidden: action.hidden }]

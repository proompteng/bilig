...
describe("workbook import", () => {
  it("classifies supported csv and xlsx workbook files", () => {
    expect(
      resolveWorkbookImportContentType(
        new File(["alpha"], "metrics.csv", { type: CSV_CONTENT_TYPE }),
      ),
    ).toBe(CSV_CONTENT_TYPE);
    expect(
      resolveWorkbookImportContentType(
        new File(["alpha"], "model.xlsx", { type: XLSX_CONTENT_TYPE }),
      ),
    ).toBe(XLSX_CONTENT_TYPE);
  ...
  it("stages a local preview and imports a new workbook through the authoritative loader", async () => {
    ...
    await act(async () => {
      host
        .querySelector("[data-testid='workbook-import-toggle']")
    ...
    await act(async () => {
      host
        .querySelector("[data-testid='workbook-import-create']")
        ?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
  ...
  it("routes replace-current imports through the current document id and surfaces unsupported files", async () => {
    ...
    await act(async () => {
      host
        .querySelector("[data-testid='workbook-import-replace']")
        ?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });

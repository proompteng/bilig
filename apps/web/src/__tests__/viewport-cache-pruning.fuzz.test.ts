  ...
  .chain((entries) =>
    fc
      .record({
        ...
        accessTicks: fc.dictionary(fc.constantFrom(...entries.map((entry) => entry.key)), fc.integer({ min: 0, max: 50 })),
      })
      .map((raw) => {
...
function referenceEviction(args: Parameters<typeof selectProjectedViewportKeysToEvict>[0]): string[] {
  ...
  intactCandidates.sort((left, right) => left.accessTick - right.accessTick || left.key.localeCompare(right.key))
  return [...missingSnapshotKeys, ...intactCandidates.map((candidate) => candidate.key)].slice(0, overflow)
}

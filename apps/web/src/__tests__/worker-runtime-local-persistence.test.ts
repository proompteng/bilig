...
describe('worker runtime local persistence', () => {
  it('reuses projection exports when authoritative state is shared and no pending mutations remain', async () => {
    ...
    const cache = {
      getAuthoritativeSnapshot: vi.fn((input) =>
        input.canReuseProjectionState ? input.exportProjectionSnapshot() : input.exportAuthoritativeSnapshot(),
      ),
      getAuthoritativeReplica: vi.fn((input) =>
        input.canReuseProjectionState ? input.exportProjectionReplica() : input.exportAuthoritativeReplica(),
      ),
    }
    ...
    expect(cache.getAuthoritativeSnapshot).toHaveBeenCalledWith(expect.objectContaining({ canReuseProjectionState: true }))
    expect(cache.getAuthoritativeReplica).toHaveBeenCalledWith(expect.objectContaining({ canReuseProjectionState: true }))
  })
  ...
  it('forces authoritative exports when pending mutations remain', async () => {
    ...
    expect(cache.getAuthoritativeSnapshot).toHaveBeenCalledWith(expect.objectContaining({ canReuseProjectionState: false }))
    expect(cache.getAuthoritativeReplica).toHaveBeenCalledWith(expect.objectContaining({ canReuseProjectionState: false }))

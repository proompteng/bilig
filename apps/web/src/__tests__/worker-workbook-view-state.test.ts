...
describe('worker workbook view state', () => {
  it('returns stable empty viewport snapshots before the worker handle exists', () => {
    expect(readViewportColumnWidths(undefined, 'Sheet1')).toBe(readViewportColumnWidths(undefined, 'Sheet1'))
    expect(readViewportRowHeights(undefined, 'Sheet1')).toBe(readViewportRowHeights(undefined, 'Sheet1'))
    expect(readViewportHiddenColumns(undefined, 'Sheet1')).toBe(readViewportHiddenColumns(undefined, 'Sheet1'))
    expect(readViewportHiddenRows(undefined, 'Sheet1')).toBe(readViewportHiddenRows(undefined, 'Sheet1'))
  })

...
export async function applyRuntimeSyncAction(
  ...
  switch (action.kind) {
    ...
    case "submit": {
      const target = model.pending.find(
        (mutation) => mutation.status === "local" || mutation.status === "rebased",
      );
      if (!target) {
        return;
      }
...
export function assertRuntimeSyncState(
  runtime: WorkbookWorkerRuntime,
  model: RuntimeSyncModel,
): void {
  ...
  runtimeSyncTrackedAddresses.forEach((address) => {
    ...
    if (actual.tag !== ValueTag.Number || actual.value !== expected) {
      throw new Error(
        `Runtime sync mismatch at ${address}: expected ${String(expected)}, received ${JSON.stringify(actual)}`,
      );
    }
  });
  ...
  if (JSON.stringify(runtimePending) !== JSON.stringify(modelPending)) {
    throw new Error(
      `Pending mutation mismatch: expected ${JSON.stringify(modelPending)}, received ${JSON.stringify(runtimePending)}`,
    );
  }
}
...
function expectEmptyValue(
  value: ReturnType<WorkbookWorkerRuntime["getCell"]>["value"],
  address: string,
): void {

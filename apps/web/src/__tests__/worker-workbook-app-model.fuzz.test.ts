...
import { ErrorCode, ValueTag, type CellSnapshot, type WorkbookDefinedNameSnapshot } from '@bilig/protocol'
...
describe('worker workbook app model fuzz', () => {
  ...
  it('resolves defined-name cell refs and clamps movement within workbook bounds', async () => {
    await runProperty({
      ...
      arbitrary: fc.record({
        targetName: fc.string({ minLength: 1, maxLength: 24 }).filter((value) => value.trim().length > 0),
        sheetName: fc.constantFrom('Sheet1', 'Sheet2', 'Summary'),
        address: fc.constantFrom('A1', 'B2', 'C3', 'D4'),
        movement: fc.tuple(fc.constantFrom<-1 | 0 | 1>(-1, 0, 1), fc.constantFrom<-1 | 0 | 1>(-1, 0, 1)),
      }),

...
interface ConflictHarnessSpies {
  readonly applyParsedInput: ReturnType<
    typeof vi.fn<
      (sheetName: string, address: string, parsed: ParsedEditorInput) => Promise<void>
    >
  >;
  readonly finishEditingWithAuthoritative: ReturnType<
    typeof vi.fn<(targetSelection: WorkerRuntimeSelection) => void>
  >;
  readonly resetEditorConflictTracking: ReturnType<
    typeof vi.fn<(nextSelection?: WorkerRuntimeSelection) => void>
  >;
...
function ConflictHarness(props: {
  ...
  const [editorSelectionBehavior, setEditorSelectionBehavior] =
    useState<EditSelectionBehavior>("caret-end");
...
describe("workbook editor conflict", () => {
  it("surfaces a stale same-cell draft and applies the local draft through the compare flow", async () => {
    ...
    expect(badgeBanner?.textContent).toContain(
      "Remote update detected in Sheet1!A1 while you were editing.",
    );

    await act(async () => {
      host
        .querySelector("[data-testid='editor-conflict-review']")
        ?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });

    expect(host.querySelector("[data-testid='editor-conflict-apply-mine']")).toBeTruthy();
    expect(host.querySelector("[data-testid='editor-conflict-banner']")?.textContent).toContain(
      "Sheet1!A1 changed while you were editing",
    );
    expect(host.querySelector("[data-testid='editor-conflict-banner']")?.textContent).toContain(
      "remote",
    );
    ...
    await act(async () => {
      host
        .querySelector("[data-testid='editor-conflict-apply-mine']")
        ?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    ...
    expect(host.querySelector("[data-testid='selection-behavior']")?.textContent).toBe(
      "select-all",
    );

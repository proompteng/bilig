...
describe("workbook mutation journal", () => {
  ...
  it("moves an active mutation through submitted, failed, retry, and acked states", () => {
    const submitted = Effect.runSync(
      markPendingWorkbookMutationSubmitted(
        Effect.runSync(recordPendingWorkbookMutationAttempt(createMutation(), 150)),
        150,
      ),
    );
    const failed = Effect.runSync(
      markPendingWorkbookMutationFailed(submitted, "mutation rejected", 175),
    );
    ...
    const acked = Effect.runSync(
      markPendingWorkbookMutationAcked(
        Effect.runSync(
          markPendingWorkbookMutationSubmitted(
            Effect.runSync(recordPendingWorkbookMutationAttempt(retried, 210)),
            210,
          ),
        ),
        260,

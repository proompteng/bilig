...
function ConflictValueBlock(props: {
  ...
  return (
    <div className="min-w-0 rounded-[var(--wb-radius-control)] border border-[var(--wb-border)] bg-[var(--wb-surface)] px-3 py-3">
      <div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--wb-text-subtle)]">
        {props.label}
      </div>
...
export function WorkbookEditorConflictBanner(props: {
  ...
  if (props.conflict.phase === "badge") {
    return (
      <div
        className="border-b border-[var(--wb-accent-ring)] bg-[var(--wb-accent-soft)] px-3 py-2"
        data-testid="editor-conflict-banner"
      >
        <div className="flex flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1 text-sm text-[var(--wb-accent)]">
            Remote update detected in {targetLabel} while you were editing. Your draft is still
            local.
          </div>
          <button
  ...
  return (
    <div
      className="border-b border-[var(--wb-accent-ring)] bg-[var(--wb-accent-soft)] px-3 py-3"
      data-testid="editor-conflict-banner"
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            <div className="text-[13px] font-semibold text-[var(--wb-accent)]">
              {targetLabel} changed while you were editing
            </div>
            <div className="mt-1 text-[12px] text-[var(--wb-text-subtle)]">
              Compare your local draft with the latest authoritative value before deciding what to
              keep.
            </div>
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <ConflictValueBlock
            ...
            secondaryValue={
              authoritativeResolvedValue !== authoritativeEditorValue
                ? authoritativeResolvedValue
                : undefined
            }
          />
        </div>

...
function stripLeadingMarkdownTitle(markdown: string, title: string | null | undefined): string {
  ...
  if (!firstLine || normalizeMarkdownHeading(firstLine[1] ?? '') !== normalizeMarkdownHeading(title)) {
    return markdown
  }
...
function renderBlockTokens(tokens: readonly Token[] | undefined, keyPrefix: string, tone: WorkbookMarkdownTone): ReactNode[] {
  if (!tokens || tokens.length === 0) {
    return []
  }
  ...
  return tokens.flatMap((token, index) => {
    ...
    switch (token.type) {
      ...
      case 'heading': {
        ...
        const className = depth <= 2 ? `text-[13px] font-semibold ${bodyClass}` : `text-[12px] font-semibold ${bodyClass}`
        return (

...
export function clonePendingWorkbookMutation(mutation: PendingWorkbookMutation): PendingWorkbookMutation {
  return {
    ...mutation,
    args: [...mutation.args],
  }
...
export function isPendingWorkbookMutationReadyForSubmission(mutation: PendingWorkbookMutation): boolean {
...
export function recordPendingWorkbookMutationAttempt(
  ...
  return transitionMutation(() => {
    ...
    const nextStatus = mutation.status === 'failed' ? (mutation.rebasedAtUnixMs === null ? 'local' : 'rebased') : mutation.status
...
export function markPendingWorkbookMutationSubmitted(
  ...
  return transitionMutation(() => {
    if (mutation.status !== 'local' && mutation.status !== 'rebased' && mutation.status !== 'failed') {
      throw new MutationJournalTransitionError({

...
export class WorkerViewportPatchPublisher {
  ...
  subscribe(
    ...
    listener(
      encodeViewportPatch(
        this.options.buildPatch(state, null, this.options.getCurrentMetrics(), null),
      ),
    );
  ...
  buildPatch(
    ...
    if (
      ...
      this.options.canReadLocalProjectionForViewport()
    ) {
      const localBase = this.options.readLocalViewport(
  ...
  broadcast(input: {
    ...
    const impactedSheets =
      input.impactsBySheet === null ? null : new Set(input.impactsBySheet.keys());

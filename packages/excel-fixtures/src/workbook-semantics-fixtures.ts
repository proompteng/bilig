...
export const canonicalWorkbookSemanticsFixtures: readonly ExcelFixtureCase[] = [
  fixture("names", "defined-name-case-insensitive", "Defined names resolve case-insensitively", "=taxrate*A1", [input("A1", 100)], [output("A2", numberExpected(8.5))], {
  ...
  fixture("names", "defined-name-multi-scalar-pack", "Multiple workbook names participate in one scalar expression", "=TaxRate+FeeRate", [], [output("A1", numberExpected(0.1))], {
    definedNames: ratePack,
    notes: "Captures the common workbook metadata case where more than one scalar name is bound at once."
  }),
  fixture("names", "defined-name-missing", "Missing workbook names surface #NAME?", "=MissingRate*A1", [input("A1", 100)], [output("A2", errorExpected(ErrorCode.Name, "#NAME?"))], {
    notes: "Keeps fixture coverage aligned with the JS evaluator and engine name-miss behavior."
  }),
  fixture("arithmetic", "cross-sheet-multiply", "Cross-sheet scalar arithmetic", "=Sheet2!B1*3", [input("B1", 4, { sheetName: "Sheet2" })], [output("A1", numberExpected(12))], {
  ...
  fixture(
    ...
    "=Sheet2!B1*3",
    [input("B1", null, { sheetName: "Sheet2", note: "Marks Sheet2 as present while leaving the referenced cell empty." })],
    [output("A1", numberExpected(0))]
  ),
  fixture("arithmetic", "missing-sheet-ref-error", "Missing cross-sheet cells surface #REF!", "=Sheet2!B1*3", [], [output("A1", errorExpected(ErrorCode.Ref, "#REF!"))], {
    notes: "The target worksheet is intentionally absent so the harness can capture unresolved-sheet error behavior."
  }),
  fixture("aggregation", "cross-sheet-range-sum", "Cross-sheet range aggregation", "=SUM(Sheet2!A1:A2)", [input("A1", 2, { sheetName: "Sheet2" }), input("A2", 3, { sheetName: "Sheet2" })], [output("A1", numberExpected(5))], {
  ...
  fixture("aggregation", "missing-sheet-range-ref-error", "Missing cross-sheet ranges surface #REF!", "=SUM(Sheet2!A1:A2)", [], [output("A1", errorExpected(ErrorCode.Ref, "#REF!"))], {

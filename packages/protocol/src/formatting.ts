...
export function normalizeCellNumberFormatPreset(
  input: CellNumberFormatPreset,
): CellNumberFormatPreset {
  switch (input.kind) {
    case "general":
    case "text":
...
export function createCellNumberFormatRecord(
  id: string,
  input: CellNumberFormatInput,
): CellNumberFormatRecord {
...
function formatSignedCurrency(
  ...
  const rendered = formatter.format(
    value < 0 && format.negativeStyle !== "parentheses" ? absolute : value,
  );

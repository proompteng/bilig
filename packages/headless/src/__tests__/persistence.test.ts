...
describe("WorkPaper persistence helpers", () => {
  it("roundtrips sheets, named expressions, config, and sheet order through the persisted document format", () => {
    ...
    const restored = createWorkPaperFromDocument(
      parseWorkPaperDocument(serializeWorkPaperDocument(document)),
    );
    ...
    expect(restored.getAllNamedExpressionsSerialized()).toEqual(
      workbook.getAllNamedExpressionsSerialized(),
    );

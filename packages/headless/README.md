...
```sh
npm exec --package @bilig/headless@0.40.5 -- bilig-formula-clinic ./reduced.xlsx --cells "Summary!B7,Inputs!B2"
```

Handing a spreadsheet task to another coding agent?

```sh
npm exec --package @bilig/headless@0.40.5 -- bilig-agent-challenge
npm exec --package @bilig/headless@0.40.5 -- bilig-mcp-challenge
```
...
Current checked npm footprint for `@bilig/headless@0.40.5`:

- Pack dry run: `637 kB` tarball, `3.90 MB` unpacked, `633` package entries.
...
npm exec --package @bilig/headless@0.40.5 -- bilig-workpaper-mcp --workpaper ./pricing.workpaper.json --init-demo-workpaper --writable
...
- Agent and MCP workflows:
  ...
  ([download](https://github.com/proompteng/bilig/releases/download/libraries-v0.40.5/bilig-workpaper.mcpb)).
- Choosing the stack:

...
export type AggregateEvaluator = (subset: readonly CellValue[], totalSet?: readonly CellValue[]) => CellValue
...
function normalizeFilterMask(filterArray: MatrixValue | undefined, totalRows: number, dataStartRow: number): boolean[] | undefined {
...
function normalizeSortOrders(sortOrder: readonly number[] | undefined): Array<{ index: number; descending: boolean }> {
...
export function evaluateGroupBy(rowFields: MatrixValue, values: MatrixValue, options: GroupByOptions): ArrayValue | CellValue {
  if (rowFields.rows !== values.rows || rowFields.rows === 0 || rowFields.cols === 0 || values.cols === 0) {
...
export function evaluatePivotBy(
  ...
  const headerRow: CellValue[] = [
    ...(headers.showFieldLabels ? rowLabels.map(stringValue) : Array.from({ length: rowFields.cols }, () => emptyValue())),
  ...
  const buildDataRow = (rowKey: string, rowValues: CellValue[], rowSubsets: CellValue[][]): CellValue[] => {

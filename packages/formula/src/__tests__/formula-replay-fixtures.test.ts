...
function expectTranslationFixture(
  ...
  const translated = translateFormulaReferences(canonical, translation.rowDelta, translation.colDelta)
  expect(translated).toBe(translation.translated)
  expect(translateFormulaReferences(translated, -translation.rowDelta, -translation.colDelta)).toBe(translation.restored)
}

function expectRenameFixture(canonical: string, rename: NonNullable<ReturnType<typeof loadFormulaReplayFixtures>[number]['rename']>): void {
  ...
  expect(renameFormulaSheetReferences(renamed, rename.newSheetName, rename.oldSheetName)).toBe(rename.restored)
}
...
function expectStructuralFixture(
  ...
  expect(
    rewriteFormulaForStructuralTransform(rewritten, structural.ownerSheetName, structural.targetSheetName, {

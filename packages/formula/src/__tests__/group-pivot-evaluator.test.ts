...
function sumAggregate(subset: readonly CellValue[]): CellValue {
  return number(
    subset.reduce((total, value) => total + (value.tag === ValueTag.Number ? value.value : 0), 0),
  );
}

function sumCells(values: readonly CellValue[] | undefined): number {
  return (values ?? []).reduce(
    (total, value) => total + (value.tag === ValueTag.Number ? value.value : 0),
    0,
  );
}
...
describe("group-pivot evaluator", () => {
  it("rejects malformed GROUPBY shapes and options", () => {
    ...
    expect(evaluateGroupBy(rows, values, { aggregate: sumAggregate })).toEqual(
    ...
    expect(
      evaluateGroupBy(
        ...
        { aggregate: sumAggregate, fieldRelationship: 2 },
      ),
    ...
    expect(
      evaluateGroupBy(
        matrix(2, 1, [text("Region"), text("East")]),
        matrix(2, 1, [text("Sales"), number(10)]),
        { aggregate: sumAggregate, fieldHeadersMode: 4 },
      ),
    ).toEqual(error(ErrorCode.Value));
    expect(
      evaluateGroupBy(
  ...
  it("keeps GROUPBY insertion order stable when aggregate sort keys tie", () => {
    const result = evaluateGroupBy(
      matrix(2, 1, [text("East"), text("West")]),
      matrix(2, 1, [number(1), number(1)]),
      {
        aggregate: sumAggregate,
        fieldHeadersMode: 0,
        totalDepth: 0,
  ...
  it("rejects malformed PIVOTBY shapes and options", () => {
    ...
    expect(evaluatePivotBy(rowFields, colFields, values, { aggregate: sumAggregate })).toEqual(
      error(ErrorCode.Value),
    );
    expect(
      evaluatePivotBy(
  ...
  it("switches PIVOTBY relative totals between row, grand, and column scopes", () => {
    ...
    const aggregate = (_subset: readonly CellValue[], totalSet?: readonly CellValue[]) =>
      number(sumCells(totalSet));

...
describe("translateCompiledFormulaWithoutAst", () => {
  ...
  it("translates axis ranges and call argument refs when parsed metadata is present", () => {
    ...
    const countIf = translateCompiledFormulaWithoutAst(
      compileFormula("COUNTIF(A1:A3,B1)"),
      1,
      2,
      "COUNTIF(C2:C4,D2)",
    );
  ...
  it("translates lookup instructions and nested lambda bodies without an AST", () => {
    const lookup = translateCompiledFormulaWithoutAst(
      compileFormula("MATCH(A6,A1:A10,0)"),
      1,
      0,
      "MATCH(A7,A2:A11,0)",
    );
    ...
    const translatedManual = translateCompiledFormulaWithoutAst(
      manualCompiled,
      1,
      2,
      "WRAP(C2:D3,C2,3,E,LAMBDA(x,C2))",
    );

...
describe("binder wasm rules", () => {
  ...
  it("keeps representative arity and wasm-argument policies stable", () => {
    ...
    expect(
      isWasmSafeBuiltinArgs("COUNTIFS", parseFormula("COUNTIFS(A1:A4,1,B1:B4,2)").args, deps),
    ).toBe(true);
    expect(isWasmSafeBuiltinArgs("LOOKUP", parseFormula("LOOKUP(1,A1:A4,B1:B4)").args, deps)).toBe(
      true,
    );
    expect(isWasmSafeBuiltinArgs("LOOKUP", parseFormula("LOOKUP(A1:A2,A1:A4)").args, deps)).toBe(
      false,
    );
    expect(
      isWasmSafeBuiltinArgs("TEXTJOIN", parseFormula('TEXTJOIN(",",TRUE,A1:A4)').args, deps),
    ).toBe(true);
  });
});

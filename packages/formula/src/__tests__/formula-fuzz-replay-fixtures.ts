...
export interface FormulaReplayEvaluationFixture {
  expected: { kind: 'number'; value: number } | { kind: 'string'; value: string } | { kind: 'boolean'; value: boolean }
}
...
function parseTranslation(value: unknown, fileName: string): FormulaReplayTranslationFixture | undefined {
  if (value === undefined) {
    return undefined
  }
  if (
...
function parseStructural(value: unknown, fileName: string): FormulaReplayStructuralFixture | undefined {
  if (value === undefined) {
    return undefined
  }
  if (
...
function parseEvaluation(value: unknown, fileName: string): FormulaReplayEvaluationFixture | undefined {
  if (value === undefined) {
    return undefined
  }
  if (!isRecord(value) || !isRecord(value['expected']) || typeof value['expected']['kind'] !== 'string') {

...
describe("js evaluator array specials", () => {
  it("evaluates spill-oriented array helpers", () => {
    ...
    expect(
      evaluatePlanResult(lowerToPlan(parseFormula('TEXTSPLIT("red,blue|green",",","|")')), context),
    ).toEqual({
      kind: "array",
      rows: 2,
      cols: 2,
  ...
  it("preserves validation errors for array special calls", () => {
    ...
    expect(
      evaluatePlan(lowerToPlan(parseFormula("MAKEARRAY(2,2,LAMBDA(r,c,SEQUENCE(2)))")), context),
    ).toEqual(err(ErrorCode.Value));
    expect(
      evaluatePlan(lowerToPlan(parseFormula("BYROW(A1:B2,LAMBDA(r,SEQUENCE(2)))")), context),
    ).toEqual(err(ErrorCode.Value));
  });
});

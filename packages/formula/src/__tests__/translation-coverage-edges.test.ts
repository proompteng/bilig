...
function makeExactMatchInstruction(
...
function makeApproximateMatchInstruction(
...
function withInvalidTransformKind(
...
describe("translation coverage edges", () => {
  it("builds template keys for literal, invalid, and invoke-heavy AST shapes", () => {
    expect(buildRelativeFormulaTemplateKeyFromAst({ kind: "NumberLiteral", value: 7 }, 0, 0)).toBe(
      "n:7",
    );
    expect(
      buildRelativeFormulaTemplateKeyFromAst({ kind: "BooleanLiteral", value: false }, 0, 0),
    ).toBe("b:0");
    expect(
      buildRelativeFormulaTemplateKeyFromAst({ kind: "StringLiteral", value: "text" }, 0, 0),
    ).toBe('s:"text"');
    expect(
      buildRelativeFormulaTemplateKeyFromAst({ kind: "ErrorLiteral", code: ErrorCode.Ref }, 0, 0),
    ).toBe(`e:${ErrorCode.Ref}`);
    expect(buildRelativeFormulaTemplateKeyFromAst({ kind: "NameRef", name: "Rate" }, 0, 0)).toBe(
      "name:Rate",
    );
    expect(
      buildRelativeFormulaTemplateKeyFromAst(
    ...
    ).toBe("table:Sales[Amount]");
    expect(
      buildRelativeFormulaTemplateKeyFromAst({ kind: "CellRef", ref: "A0" } as FormulaNode, 0, 0),
    ).toBe("cell:.:invalid:A0");
    expect(buildRelativeFormulaTemplateKeyFromAst({ kind: "SpillRef", ref: "B2" }, 0, 0)).toBe(
      "spill:.:rc1:rr1",
    );
    expect(
      buildRelativeFormulaTemplateKeyFromAst({ kind: "ColumnRef", ref: "?" } as FormulaNode, 0, 0),
    ).toBe("col:.:invalid:?");
    expect(buildRelativeFormulaTemplateKeyFromAst({ kind: "RowRef", ref: "3" }, 0, 0)).toBe(
      "row:.:r2",
    );
    expect(
      buildRelativeFormulaTemplateKeyFromAst(
        { kind: "RangeRef", refKind: "rows", start: "2", end: "4" },
        0,
        0,
      ),
    ).toBe("range:rows:.:r1:r3");
    expect(
      buildRelativeFormulaTemplateKeyFromAst(
        { kind: "RangeRef", refKind: "cols", start: "B", end: "D" },
        0,
        0,
      ),
    ).toBe("range:cols:.:r1:r3");
    expect(
      buildRelativeFormulaTemplateKeyFromAst(
        { kind: "UnaryExpr", operator: "-", argument: { kind: "NameRef", name: "Rate" } },
        0,
        0,
      ),
  ...
  it("rewrites manual compiled formulas through axis error, raw fallback, and parsed fallback branches", () => {
    ...
    const rowInserted = rewriteCompiledFormulaForStructuralTransform(
      rowCompiled,
      "Sheet1",
      "Sheet1",
      {
    ...
    const colInserted = rewriteCompiledFormulaForStructuralTransform(
      colCompiled,
      "Sheet1",
      "Sheet1",
      {
    ...
    const rowDeleted = rewriteCompiledFormulaForStructuralTransform(
      rowCompiled,
      "Sheet1",
      "Sheet1",
      {
    ...
    const rawFallback = rewriteCompiledFormulaForStructuralTransform(
      rawFallbackCompiled,
      "Sheet1",
      "Sheet1",
      {
    ...
    const rowLookupRewritten = rewriteCompiledFormulaForStructuralTransform(
      rowLookupCompiled,
      "Sheet1",
      "Sheet1",
      {
    ...
    expect(rawFallback.compiled.parsedSymbolicRanges).toEqual(
      rawFallbackCompiled.parsedSymbolicRanges,
    );
  ...
  it("throws for invalid manual references and unsupported structural transforms", () => {
    const unsupportedTransform = withInvalidTransformKind(
      { kind: "insert", axis: "row", start: 0, count: 1 },
      "noop",
    );

    expect(() =>
      translateCompiledFormula(
        makeCompiledFormula({ kind: "CellRef", ref: "A0" } as FormulaNode),
        0,
        0,
      ),
    ).toThrow("Invalid cell reference 'A0'");
    expect(() =>
      translateCompiledFormula(
        makeCompiledFormula({ kind: "ColumnRef", ref: "1" } as FormulaNode),
        0,
        0,
      ),
    ).toThrow("Invalid column reference '1'");
    expect(() =>
      translateCompiledFormula(
        makeCompiledFormula({ kind: "RowRef", ref: "A" } as FormulaNode),
        0,
        0,
      ),
    ).toThrow("Invalid row reference 'A'");
    expect(() => rewriteAddressForStructuralTransform("A1", unsupportedTransform)).toThrow(
      "Unexpected value: [object Object]",
    );
    expect(() => rewriteRangeForStructuralTransform("A1", "A2", unsupportedTransform)).toThrow(
      "Unexpected value: [object Object]",
    );
  });
});

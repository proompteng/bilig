...
describe("addressing helpers", () => {
  ...
  it("parses cells and ranges with quoted sheet names and normalized ordering", () => {
    ...
    expect(formatRangeAddress(parseRangeAddress("'Ops Team'!D:B", "Ignored"))).toBe(
      "'Ops Team'!B:D",
    );
  ...
  it("rejects invalid cell and range addresses", () => {
    ...
    expect(() => parseRangeAddress("A1:B")).toThrow(
      "Range endpoints must use the same reference type",
    );
    expect(() => parseRangeAddress("Sheet1!A1:Sheet2!B2")).toThrow(
      "Range endpoints must target the same sheet",
    );

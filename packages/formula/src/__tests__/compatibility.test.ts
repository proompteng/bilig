...
describe('formula compatibility registry', () => {
  it('keeps the generated formula dominance snapshot aligned with inventory and canonical status', () => {
    ...
    expect(snapshot.formulaBreadth.officeListed).toEqual(
      createRatioRecord(formulaInventorySummary.officeListedRegisteredInCodebase, formulaInventorySummary.officeListed),
    )
    expect(snapshot.formulaBreadth.tracked).toEqual(
      createRatioRecord(formulaInventorySummary.registeredInCodebase, formulaInventorySummary.total),
    )
    ...
    expect(snapshot.canonical.nonProductionRows.map((row) => row.id).toSorted()).toEqual(canonicalOpenRows)
...
function isFormulaDominanceSnapshotFixture(value: unknown): value is FormulaDominanceSnapshotFixture {
  ...
  return (
    ...
    value['canonical']['nonProductionRows'].every((row) => isRecord(row) && typeof row['id'] === 'string')
  )
}
...
function isRatio(value: unknown): value is { percent: number; production: number; total: number } {
  return (
    isRecord(value) && typeof value['percent'] === 'number' && typeof value['production'] === 'number' && typeof value['total'] === 'number'
  )
}

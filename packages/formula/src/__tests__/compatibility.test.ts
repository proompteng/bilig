...
import { excelFixtureFamilies, excelFixtureIdPattern, excelTop100CanonicalFixtures, excelTop100SmokeSuite } from "../../../excel-fixtures/src/index.js";

describe("formula compatibility registry", () => {
  it("keeps the canonical Top 100 fixture corpus and registry in exact lockstep", () => {
    expect(excelTop100CanonicalFixtures).toHaveLength(100);
    expect(formulaCompatibilityRegistry).toHaveLength(100);

    const fixtureIds = new Set(excelTop100CanonicalFixtures.map((fixture) => fixture.id));
    const registryIds = new Set(formulaCompatibilityRegistry.map((entry) => entry.id));

    expect([...fixtureIds].sort()).toEqual([...registryIds].sort());
  });

  it("uses unique, status-friendly fixture ids in the canonical Top 100 pack", () => {
    const ids = excelTop100CanonicalFixtures.map((fixture) => fixture.id);
  ...
  it("keeps family, formula, and metadata aligned across fixtures and registry", () => {
    const fixtureMap = new Map(excelTop100CanonicalFixtures.map((fixture) => [fixture.id, fixture]));
    ...
    formulaCompatibilityRegistry.forEach((entry) => {
      ...
      expect(compatibilityMilestones).toContain(entry.milestone);
  ...
  it("covers every declared family in both the fixture and registry layers", () => {
    const fixtureFamiliesSeen = new Set(excelTop100CanonicalFixtures.map((fixture) => fixture.family));
  ...
  it("retains a top100 smoke suite derived from the canonical export", () => {
    expect(excelTop100SmokeSuite.id).toBe("top100-smoke");
    expect(excelTop100SmokeSuite.cases).toHaveLength(5);
    expect(excelTop100SmokeSuite.cases).toEqual(excelTop100CanonicalFixtures.slice(0, 5));
  });

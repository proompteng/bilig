...
describe('lookup matrix builtins', () => {
  ...
  it('returns value errors for missing required matrix arguments instead of throwing', () => {
    ...
    expect(Reflect.apply(SUMX2MY2, undefined, [cellRange([num(1)], 1, 1)])).toEqual(err(ErrorCode.Value))
    expect(Reflect.apply(MDETERM, undefined, [])).toEqual(err(ErrorCode.Value))
    expect(Reflect.apply(MMULT, undefined, [cellRange([num(1)], 1, 1)])).toEqual(err(ErrorCode.Value))
    expect(Reflect.apply(PERCENTOF, undefined, [cellRange([num(1)], 1, 1)])).toEqual(err(ErrorCode.Value))
  })
})

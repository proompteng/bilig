...
describe("js evaluator workbook special calls", () => {
  it("evaluates GETPIVOTDATA and INDIRECT through workbook hooks", () => {
    const context = {
      ...
      resolveCell: (_sheetName: string, address: string): CellValue =>
        address === "A1" ? number(2) : { tag: ValueTag.Empty },
    ...
    expect(evaluateAst(parseFormula('GETPIVOTDATA("Sales",B2,"Region","East")'), context)).toEqual(
      number(15),
    );
  ...
  it("evaluates workbook-shaped group and multiple-operation helpers", () => {
    const context = {
      sheetName: "Sheet1",
      resolveCell: (_sheetName: string, address: string): CellValue =>
        address === "B5" ? number(0) : { tag: ValueTag.Empty },
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] => {
        if (start === "A1" && end === "A5") {
    ...
    expect(
      evaluateAstResult(parseFormula("PIVOTBY(A1:A5,B1:B5,C1:C5,SUM,3,1,0,1)"), context),
    ).toEqual({
      kind: "array",
      rows: 4,
      cols: 4,
    ...
    expect(evaluateAst(parseFormula("MULTIPLE.OPERATIONS(B5,B3,C4,B2,D2)"), context)).toEqual(
      number(5),
    );
  });

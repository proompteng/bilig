...
export function rewriteCompiledFormulaForStructuralTransform(
  ...
  return {
    ...
    compiled: {
      ...
      symbolicRefs: compiled.symbolicRefs.map((ref) =>
        rewriteQualifiedCellReference(ref, ownerSheetName, targetSheetName, transform),
      ),
      ...
      jsPlan: compiled.jsPlan.map((instruction) =>
        rewriteJsPlanInstruction(instruction, ownerSheetName, targetSheetName, transform),
      ),
      ...(compiled.parsedDeps
        ? {
...
function rewriteRangeAddressForStructuralTransform(
  range: RangeAddress,
  transform: StructuralAxisTransform,
    ...
    case "cells": {
      ...
      return parseRangeAddress(
        formatQualifiedRangeReference(
          range.sheetName,
          nextRange.startAddress,
          nextRange.endAddress,
        ),
      );
    }
    case "rows":
...
function rewriteJsPlanInstruction(
  ...
  switch (instruction.opcode) {
    case "push-cell":
      return {
        ...instruction,
        address: rewriteReferenceOperandAddress(
    ...
    case "push-lambda":
      return {
        ...
        body: instruction.body.map((step) =>
          rewriteJsPlanInstruction(step, ownerSheetName, targetSheetName, transform),
        ),
      };
    case "push-number":
    case "push-boolean":
...
function rewritePlanRangeInstruction(
  ...
  if (
    (refKind === "rows" && transform.axis !== "row") ||

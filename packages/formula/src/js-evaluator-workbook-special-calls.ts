...
export function evaluateWorkbookSpecialCall(
  ...
  switch (callee) {
    ...
    case "GROUPBY": {
      ...
      const sortOrder =
        deps.vectorIntegerArgument(rawArgs[5]) ??
    ...
    case "PIVOTBY": {
      ...
      const rowSortOrder =
        deps.vectorIntegerArgument(rawArgs[6]) ??
        (rawArgs[6] ? [deps.scalarIntegerArgument(rawArgs[6]) ?? Number.NaN] : undefined);
      const colSortOrder =
        deps.vectorIntegerArgument(rawArgs[8]) ??
    ...
    case "MULTIPLE.OPERATIONS": {
      ...
      const columnCellAddress =
        rawArgs.length === 5 ? deps.referenceTopLeftAddress(argRefs[3]) : undefined;
      const columnCellSheetName =
        rawArgs.length === 5 ? deps.referenceSheetName(argRefs[3], context) : undefined;
      const columnReplacementAddress =
        rawArgs.length === 5 ? deps.referenceTopLeftAddress(argRefs[4]) : undefined;
      const columnReplacementSheetName =
        rawArgs.length === 5 ? deps.referenceSheetName(argRefs[4], context) : undefined;
      if (
        rawArgs.length === 5 &&
        (!columnCellAddress ||
          !columnCellSheetName ||
    ...
    case "INDIRECT": {
      ...
      try {
        ...
        return deps.stackScalar(
          context.resolveCell(cell.sheetName ?? context.sheetName, cell.text),
        );
      } catch {
        // fall through
      }

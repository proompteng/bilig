...
export function evaluateArraySpecialCall(
  ...
  switch (callee) {
    ...
    case "TEXTSPLIT": {
      ...
      const rowDelimiter =
        rawArgs[2] === undefined ? undefined : deps.coerceScalarTextArgument(rawArgs[2]);
      ...
      for (let row = 1; row <= rows; row += 1) {
        for (let col = 1; col <= cols; col += 1) {
          const result = deps.applyLambda(
            lambda,
            [
              deps.stackScalar({ tag: ValueTag.Number, value: row }),
              deps.stackScalar({ tag: ValueTag.Number, value: col }),
            ],
            context,
          );

...
export function createFixedIncomeBuiltins({
  ...
  return {
    ...
    YIELDDISC: (settlementArg, maturityArg, priceArg, redemptionArg, basisArg) => {
      ...
      return years === undefined
        ? valueError()
        : numberResult((redemption - price) / price / years);
    },
    ...
    TBILLYIELD: (settlementArg, maturityArg, priceArg) => {
      ...
      return days === undefined
        ? valueError()
        : numberResult(((100 - price) * 360) / (price * days));
    },
    ...
    ODDLPRICE: (
      ...
      const fractions = oddLastCouponFractions(
        settlement,
        maturity,
        lastInterest,
        frequency,
    ...
    DURATION: (settlementArg, maturityArg, couponArg, yieldArg, frequencyArg, basisArg) => {
      ...
      const duration = metrics
        ? macaulayDuration(metrics, coupon, yieldRate, frequency)
        : undefined;

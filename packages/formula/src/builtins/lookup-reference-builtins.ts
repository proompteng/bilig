...
function approximateMatchAscending(
  lookupValue: CellValue,
  range: RangeBuiltinArgument,
  deps: LookupReferenceBuiltinDeps,
): number {
...
function approximateMatchDescending(
  lookupValue: CellValue,
  range: RangeBuiltinArgument,
  deps: LookupReferenceBuiltinDeps,
): number {
...
export function createLookupReferenceBuiltins(
  deps: LookupReferenceBuiltinDeps,
  ...
  return {
    ...
    LOOKUP: (lookupValue, lookupVectorArg, resultVectorArg = lookupVectorArg) => {
      ...
      const position = shouldApproximate
        ? approximateMatchAscending(lookupValue, lookupRangeOrError, deps)
        : exactPosition;
    ...
    XLOOKUP: (
      ...
      if (
        deps.isRangeArg(lookupValue) ||
        deps.isRangeArg(ifNotFound) ||
        deps.isRangeArg(matchMode) ||
        deps.isRangeArg(searchMode)
      ) {
        return deps.errorValue(ErrorCode.Value);
      }

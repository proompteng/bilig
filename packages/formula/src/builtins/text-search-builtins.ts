...
function applyReplacementTemplate(
  ...
  for (let index = 0; index < template.length; index += 1) {
    ...
    const twoDigitIndex =
      secondDigit !== undefined && /\d/.test(secondDigit) ? Number(`${next}${secondDigit}`) : -1;
...
export function createTextSearchBuiltins(
  deps: TextSearchBuiltinDeps,
  ...
  return {
    FIND: (...args) => {
      ...
      const found = findPosition(
        ...
        deps.coerceText(withinTextValue),
        start,
        true,
        false,
      );
    ...
    SEARCHB: (...args) => {
      ...
      return deps.isErrorValue(found)
        ? found
        : deps.numberResult(deps.charPositionToBytePosition(text, found));
    },
    FINDB: (...args) => {
      const existingError = deps.firstError(args);
      if (existingError) {
    ...
    REGEXREPLACE: (...args) => {
      ...
      const [textValue, patternValue, replacementValue, occurrenceValue, caseSensitivityValue] =
        args;
    ...
    TEXTBEFORE: (...args) => {
      ...
      const instanceNumber =
        instanceValue === undefined ? 1 : deps.coerceNumber(instanceValue);
      const matchMode = matchModeValue === undefined ? 0 : deps.coerceNumber(matchModeValue);
      const matchEndNumber =
        matchEndValue === undefined ? 0 : deps.coerceNumber(matchEndValue);
      if (

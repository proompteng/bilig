...
function addTemperatureUnit(
  aliases: readonly string[],
  unit: ConvertTemperatureUnit["unit"],
): void {
  for (const alias of aliases) {
  ...
  if (fromUnit.kind === "temperature" || toUnit.kind === "temperature") {
    ...
    return numberResult(
      normalizeConvertResult(
        kelvinToTemperature(toUnit.unit, temperatureToKelvin(fromUnit.unit, numeric)),
      ),
    );
  }
...
export function euroconvertBuiltin(
  ...
  const triangulationPrecision =
    triangulationPrecisionArg === undefined ? undefined : toInteger(triangulationPrecisionArg);
  if (
    numeric === undefined ||
    sourceCode === undefined ||
    targetCode === undefined ||
    fullPrecision === undefined ||
    (triangulationPrecisionArg !== undefined &&
      (triangulationPrecision === undefined || triangulationPrecision < 3))
  ) {
    return errorResult(ErrorCode.Value);
  }

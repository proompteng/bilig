...
export function buildRelativeFormulaTemplateTokenKey(
  source: string,
  ownerRow: number,
  ownerCol: number,
): string {
  ...
  for (let index = 0; index < tokens.length; ) {
    ...
    const explicitSheetMatch = matchTemplateReferenceWithExplicitSheet(
      tokens,
      index,
      ownerRow,
      ownerCol,
...
function matchTemplateReferenceWithExplicitSheet(
  ...
  if (
    (sheetToken?.kind !== "identifier" && sheetToken?.kind !== "quotedIdentifier") ||
...
function buildRelativeCellReferenceKey(
  parsed: ParsedCellReference,
  ownerRow: number,
  ownerCol: number,
): string {

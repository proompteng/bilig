...
export function evaluateContextSpecialCall(
  ...
  switch (callee) {
    ...
    case "COLUMN": {
      ...
      return deps.stackScalar(
        column === undefined ? deps.error(ErrorCode.Value) : deps.numberValue(column),
      );
    }
    case "ISOMITTED": {
      if (rawArgs.length !== 1) {
    ...
    case "PHONETIC": {
      ...
      if (target.kind === "range") {
        return deps.stackScalar(
          deps.stringValue(deps.toStringValue(target.values[0] ?? deps.emptyValue())),
        );
      }
      return deps.stackScalar(deps.error(ErrorCode.Value));
    }
    ...
    case "SHEETS": {
      ...
      return deps.stackScalar(
        deps.sheetIndexByName(scalar.value, context) === undefined
          ? deps.error(ErrorCode.NA)
          : deps.numberValue(1),
      );
    }
    case "CELL": {
      ...
      switch (normalizedInfoType) {
        ...
        case "type": {
          ...
          return deps.stackScalar(
            deps.stringValue(deps.cellTypeCode(context.resolveCell(sheetName, address))),
          );
        }
        case "filename":

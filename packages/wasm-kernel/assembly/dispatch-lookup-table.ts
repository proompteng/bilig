...
export function tryApplyLookupTableBuiltin(
  ...
  if (builtinId == BuiltinId.Index && (argc == 2 || argc == 3)) {
    ...
    if (
      ...
      return writeLookupError(
        base,
        argc == 3 && tagStack[base + 2] == ValueTag.Error
          ? <i32>valueStack[base + 2]
          : ErrorCode.Value,
        rangeIndexStack,
        valueStack,
        tagStack,
  ...
  if (builtinId == BuiltinId.Vlookup && (argc == 3 || argc == 4)) {
    ...
    const rangeLookup =
      argc == 4 ? coerceLookupBoolean(tagStack[base + 3], valueStack[base + 3]) : 1;
    if (
      rowCount <= 0 ||
      colCount <= 0 ||

...
export function tryApplyArrayReshapeBuiltin(
  ...
  if (
    (builtinId == BuiltinId.Tocol && argc >= 1 && argc <= 3) ||
    (builtinId == BuiltinId.Torow && argc >= 1 && argc <= 3)
  ) {
    ...
    const scanByCol =
      argc >= 3
        ? coerceBoolean(tagStack[base + 2], valueStack[base + 2])
        : builtinId == BuiltinId.Tocol
          ? 1
          : 0;
  ...
  if (
    (builtinId == BuiltinId.Wraprows && argc >= 2 && argc <= 4) ||
    (builtinId == BuiltinId.Wrapcols && argc >= 2 && argc <= 4)
  ) {
    const sourceKind = kindStack[base];
    if (!isArrayLike(sourceKind)) {
      return writeValueError(
    ...
    const outputRows =
      builtinId == BuiltinId.Wraprows ? (sourceLength + wrapCount - 1) / wrapCount : wrapCount;
    const outputCols =
      builtinId == BuiltinId.Wraprows ? wrapCount : (sourceLength + wrapCount - 1) / wrapCount;

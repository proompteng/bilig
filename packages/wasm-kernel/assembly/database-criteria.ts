...
export function databaseBuiltinResult(
  ...
  if (kindStack[databaseSlot] != STACK_KIND_RANGE || kindStack[criteriaSlot] != STACK_KIND_RANGE) {
    return writeDatabaseError(base, ErrorCode.Value, rangeIndexStack, valueStack, tagStack, kindStack)
  }
  ...
  if (fieldTag == ValueTag.Error) {
    return writeDatabaseError(base, <i32>fieldValue, rangeIndexStack, valueStack, tagStack, kindStack)
  }
  ...
  if (fieldTag == ValueTag.Empty) {
    ...
    if (normalizedFieldText.length == 0 && allowOmittedField) {
      ...
      for (let col = 0; col < databaseCols; col += 1) {
        ...
        const headerValue = memberScalarValue(headerMemberIndex, cellTags, cellNumbers, cellStringIds, cellErrors)
        if (headerTag == ValueTag.Error) {
          return writeDatabaseError(base, <i32>headerValue, rangeIndexStack, valueStack, tagStack, kindStack)
        }
        ...
        if (headerText != null && trimAsciiWhitespace(headerText).toUpperCase() == normalizedFieldText) {
          fieldIndex = col
          break
        }
      }
  ...
  for (let databaseRow = 1; databaseRow < databaseRows; databaseRow += 1) {
    ...
    for (let criteriaRow = 1; criteriaRow < criteriaRows; criteriaRow += 1) {
      ...
      for (let criteriaCol = 0; criteriaCol < criteriaCols; criteriaCol += 1) {
        ...
        const criteriaValue = memberScalarValue(criteriaMemberIndex, cellTags, cellNumbers, cellStringIds, cellErrors)
        if (criteriaTag == ValueTag.Empty) {
          continue
        }
        if (criteriaTag == ValueTag.Error) {
          return writeDatabaseError(base, <i32>criteriaValue, rangeIndexStack, valueStack, tagStack, kindStack)
        }
        ...
        for (let databaseCol = 0; databaseCol < databaseCols; databaseCol += 1) {
          ...
          const databaseHeaderValue = memberScalarValue(databaseHeaderMemberIndex, cellTags, cellNumbers, cellStringIds, cellErrors)
          if (databaseHeaderTag == ValueTag.Error) {
            return writeDatabaseError(base, <i32>databaseHeaderValue, rangeIndexStack, valueStack, tagStack, kindStack)
          }
          ...
          if (databaseHeaderText != null && trimAsciiWhitespace(databaseHeaderText).toUpperCase() == normalizedHeaderText) {
        ...
        if (
          !matchesCriteriaValue(
            ...
            memberScalarValue(databaseMemberIndex, cellTags, cellNumbers, cellStringIds, cellErrors),
            criteriaTag,
            criteriaValue,
            stringOffsets,
            stringLengths,
    ...
    const memberValue = memberScalarValue(fieldMemberIndex, cellTags, cellNumbers, cellStringIds, cellErrors)
  ...
  if (builtinId == BuiltinId.Dget) {
    ...
    return writeResult(base, STACK_KIND_SCALAR, dgetTag, dgetValue, rangeIndexStack, valueStack, tagStack, kindStack)
  }
  if (builtinId == BuiltinId.Daverage) {
    return numericCount == 0
      ? writeDatabaseError(base, ErrorCode.Div0, rangeIndexStack, valueStack, tagStack, kindStack)
      : writeResult(base, STACK_KIND_SCALAR, <u8>ValueTag.Number, sum / <f64>numericCount, rangeIndexStack, valueStack, tagStack, kindStack)
  }
  if (builtinId == BuiltinId.Dmax || builtinId == BuiltinId.Dmin || builtinId == BuiltinId.Dsum) {
    const value =
      builtinId == BuiltinId.Dmax ? (hasNumeric ? maximum : 0.0) : builtinId == BuiltinId.Dmin ? (hasNumeric ? minimum : 0.0) : sum
  ...
  if (numericCount == 0 || ((builtinId == BuiltinId.Dstdev || builtinId == BuiltinId.Dvar) && numericCount < 2)) {
    return writeDatabaseError(base, ErrorCode.Div0, rangeIndexStack, valueStack, tagStack, kindStack)
  }

  const mean = sum / numericCount
  const numericCountF64 = <f64>numericCount
  let variance = sumSquares - numericCount * mean * mean
  variance /= builtinId == BuiltinId.Dstdev || builtinId == BuiltinId.Dvar ? numericCountF64 - 1.0 : numericCountF64
  if (variance < 0 && variance > -1e-12) {
    variance = 0
  }
  const result = builtinId == BuiltinId.Dstdev || builtinId == BuiltinId.Dstdevp ? sqrt(variance) : variance

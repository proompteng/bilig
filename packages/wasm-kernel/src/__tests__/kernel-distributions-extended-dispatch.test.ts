...
describe('wasm kernel extended distribution dispatch', () => {
  it('keeps exponential, chi-square, beta/f, t, and discrete distribution dispatch stable', async () => {
    ...
    const packed = packPrograms([
      [encodePushNumber(0), encodePushNumber(1), encodePushBoolean(false), encodeCall(BuiltinId.Expondist, 3), encodeRet()],
      ...
      [encodePushNumber(0), encodePushNumber(1), encodePushBoolean(true), encodeCall(BuiltinId.ChisqDist, 3), encodeRet()],
      ...
      [encodePushNumber(0), encodePushNumber(1), encodePushNumber(2), encodeCall(BuiltinId.FDistRt, 3), encodeRet()],
    ...
    kernel.uploadPrograms(
      ...
      packed.offsets,
      packed.lengths,
      Uint32Array.from(Array.from({ length: 27 }, (_, index) => cellIndex(1 + Math.floor(index / width), index % width, width))),
    )
    ...
    const outputCells = Uint32Array.from(
      Array.from({ length: 27 }, (_, index) => cellIndex(1 + Math.floor(index / width), index % width, width)),
    )

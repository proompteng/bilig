...
describe('wasm kernel cashflow helpers', () => {
  ...
  it('keeps irregular cashflow helper behavior stable across refactors', async () => {
    ...
    const packed = packPrograms([
      [encodePushRange(0), encodePushNumber(0), encodePushNumber(1), encodeCall(BuiltinId.Mirr, 3), encodeRet()],
      [encodePushNumber(0), encodePushRange(1), encodePushRange(2), encodeCall(BuiltinId.Xnpv, 3), encodeRet()],
    ...
    kernel.uploadConstants(constants.constants, constants.offsets, constants.lengths)
    kernel.evalBatch(Uint32Array.from([cellIndex(3, 0, width), cellIndex(3, 1, width), cellIndex(3, 2, width)]))

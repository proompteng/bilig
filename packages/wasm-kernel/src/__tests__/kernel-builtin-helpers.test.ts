...
describe('wasm kernel builtin helper seams', () => {
  it('keeps string, spill, and coercion helper behavior stable', async () => {
    ...
    const packed = packPrograms([
      ...
      [encodePushBoolean(false), encodePushBoolean(false), encodePushNumber(0), encodeCall(BuiltinId.Or, 3), encodeRet()],
    ])
    ...
    kernel.evalBatch(Uint32Array.from([width, width + 1, width + 2, width + 3, width + 4, width + 5, width + 6]))
    ...
    expect(decodeStringValue(kernel.readStringIds()[width + 1] ?? 0, pooledStrings, kernel.readOutputStrings())).toBe("'Sheet 1'!$C$2")

    expect(kernel.readTags()[width + 2]).toBe(ValueTag.String)
    expect(decodeStringValue(kernel.readStringIds()[width + 2] ?? 0, pooledStrings, kernel.readOutputStrings())).toBe('fallback')

    expect(kernel.readTags()[width + 3]).toBe(ValueTag.String)
    expect(decodeStringValue(kernel.readStringIds()[width + 3] ?? 0, pooledStrings, kernel.readOutputStrings())).toBe('na-value')
  ...
  it('keeps statistical argument collection and error propagation stable', async () => {
    ...
    const packed = packPrograms([
      [encodePushNumber(0), encodePushBoolean(true), encodePushString(0), encodeCall(BuiltinId.Stdeva, 3), encodeRet()],
      [encodePushError(ErrorCode.Ref), encodePushNumber(1), encodeCall(BuiltinId.Stdeva, 2), encodeRet()],
      [encodePushNumber(0), encodePushBoolean(true), encodePushString(0), encodeCall(BuiltinId.Vara, 3), encodeRet()],
    ])
    kernel.uploadPrograms(packed.programs, packed.offsets, packed.lengths, Uint32Array.from([width, width + 1, width + 2]))

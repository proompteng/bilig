...
function expectNumberSpill(kernel: KernelInstance, ownerCellIndex: number, expected: readonly number[], digits = 12): void {
...
describe('wasm kernel regression dispatch slab', () => {
  it('keeps paired regression and fitted-array builtins stable across refactors', async () => {
    ...
    kernel.uploadRangeShapes(Uint32Array.from([3, 3, 2, 3, 3, 2]), Uint32Array.from([1, 1, 1, 1, 1, 1]))
    ...
    const packed = packPrograms([
      ...
      [encodePushNumber(0), encodePushRange(0), encodePushRange(1), encodeCall(BuiltinId.Forecast, 3), encodeRet()],
      [encodePushRange(0), encodePushRange(1), encodePushRange(2), encodeCall(BuiltinId.Trend, 3), encodeRet()],
      [encodePushRange(3), encodePushRange(4), encodePushRange(5), encodeCall(BuiltinId.Growth, 3), encodeRet()],

  ...
  it("keeps text formatting, conversion, and split builtins stable across refactors", async () => {
    ...
    const packed = packPrograms([
      ...
      [
        ...
        encodePushString(10),
        encodePushString(11),
        encodeCall(BuiltinId.Numbervalue, 3),
        encodeRet(),
      ],
      [
        encodePushString(12),
        encodePushNumber(0),
        encodeCall(BuiltinId.Valuetotext, 2),
        encodeRet(),
      ],
    ...
    const targets = Uint32Array.from(
      Array.from({ length: 18 }, (_, index) => cellIndex(1, index, width)),
    );
    ...
    expect(readStringCell(kernel, cellIndex(1, 12, width), pooledStrings)).toBe(
      "ยี่สิบเอ็ดบาทยี่สิบห้าสตางค์",
    );

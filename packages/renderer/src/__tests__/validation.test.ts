...
describe('validateDescriptorTree', () => {
  it('accepts valid workbook trees and empty roots', () => {
    ...
    expect(() => validateDescriptorTree(workbook([sheet('Sheet1', [cell({ addr: 'A1', value: 10 })])]))).not.toThrow()
  })

  it('rejects duplicate sheets, missing addrs, and conflicting cell props', () => {
    expect(() => validateDescriptorTree(workbook([sheet('Sheet1', [cell({ addr: 'A1', value: 1 })]), sheet('Sheet1', [])]))).toThrow(
      "Duplicate sheet name 'Sheet1'.",
    )

    expect(() => validateDescriptorTree(workbook([sheet('Sheet1', [cell({ value: 1 })])]))).toThrow('<Cell> requires an addr prop.')

    expect(() => validateDescriptorTree(workbook([sheet('Sheet1', [cell({ addr: 'A1', value: 1, formula: 'B1' })])]))).toThrow(

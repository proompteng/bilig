...
describe("validateDescriptorTree", () => {
  ...
  it("rejects duplicate sheets, missing addrs, and conflicting cell props", () => {
    expect(() =>
      validateDescriptorTree(workbook([sheet("Sheet1", [cell({ addr: "A1", value: 1 })]), sheet("Sheet1", [])]))
    ...
    expect(() =>
      validateDescriptorTree(workbook([sheet("Sheet1", [cell({ addr: "A1", value: 1, formula: "B1" })])]))

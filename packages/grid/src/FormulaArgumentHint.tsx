      ...
      <span className="flex min-w-0 items-center gap-1.5">
        {entry.args.length === 0 ? (
          ...
          entry.args.map((arg, index) => (
            <span
              className={cn('shrink-0', index === activeArgumentIndex ? 'font-semibold text-[var(--color-mauve-950)]' : undefined)}
        ...
        {entry.variadic ? <span className="shrink-0 text-[var(--color-mauve-600)]">, …</span> : null}
      </span>
      <span className="shrink-0">)</span>
    </div>
  )

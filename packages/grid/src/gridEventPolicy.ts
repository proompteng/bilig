...
}): { kind: 'ignore' } | { kind: 'autofit-column'; columnIndex: number } | { kind: 'edit-cell'; cell: Item } {
...
export function shouldSkipGridSelectionChange(options: {
  ...
  const { columnResizeActive, postDragSelectionExpiry, now, ignoreNextPointerSelection, hasDragViewport } = options

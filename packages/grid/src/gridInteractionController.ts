...
interface HandleGridBodyDoubleClickOptions extends GridInteractionCommonOptions {
  ...
  onAutofitColumn?:
    | ((this: void, columnIndex: number, fallbackWidth: number) => void | Promise<void>)
    | undefined;
...
export function handleGridPointerMove({
  ...
  if (dragHeaderSelection) {
    ...
    setGridSelection(
      resolveHeaderDragSelection(dragHeaderSelection, nextHeader.index, selectedCell).selection,
    );
    return;
  }
  ...
  if (
    dragPointerCell &&
    dragPointerCell[0] === pointerCell[0] &&
    dragPointerCell[1] === pointerCell[1]
  ) {
    return;
  }
...
export function handleGridPointerUp({
  ...
  if (dragHeaderSelection) {
    ...
    const resolvedHeaderDrag = resolveHeaderDragSelection(
      dragHeaderSelection,
      finalHeader.index,
      selectedCell,
    );
  ...
  if (dragDidMove) {
    ...
    const pointerUpResult = resolveBodyPointerUpResult(
      dragAnchorCell,
      dragPointerCell ?? dragAnchorCell,
      false,
    );
    lastBodyClickCellRef.current = pointerUpResult.clickedCell;
  }

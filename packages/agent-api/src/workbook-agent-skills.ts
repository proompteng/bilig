...
export const workbookAgentSkillDescriptors: readonly WorkbookAgentSkillDescriptor[] = [
  ...
  },
  {
    ...
    prompt: 'Inspect the current cell selection, explain its value or formula, and trace its direct precedents and dependents.',
    toolNames: [WORKBOOK_AGENT_TOOL_NAMES.getContext, WORKBOOK_AGENT_TOOL_NAMES.readSelection, WORKBOOK_AGENT_TOOL_NAMES.inspectCell],
  },
  {
  ...
  },
  {
    ...
    focus: 'analyze',
    description: 'Audit workbook integrity, formula consistency, hidden-row dependencies, used-range bloat, and performance hotspots.',
    prompt:
  ...
  },
  {
    ...
    prompt: 'Read the currently visible range and summarize its structure, headers, patterns, and any obvious issues.',
    toolNames: [
      WORKBOOK_AGENT_TOOL_NAMES.getContext,
  ...
  },
  {
    ...
    toolNames: [WORKBOOK_AGENT_TOOL_NAMES.startWorkflow, WORKBOOK_AGENT_TOOL_NAMES.readRecentChanges],
  },
  {
    id: 'edit-selection',
...
export function renderWorkbookAgentSkillInstructions(): string {
  return workbookAgentSkillDescriptors
    .map((skill) => `${skill.label} (${skill.focus}): ${skill.description} Tools: ${skill.toolNames.join(', ')}.`)
    .join(' ')
}

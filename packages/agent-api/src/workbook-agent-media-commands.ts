...
function targetRange(sheetName: string, address: string, rows: number, cols: number) {
  ...
  return {
    ...
    endAddress: formatAddress(
      anchor.row + Math.max(rows, 1) - 1,
      anchor.col + Math.max(cols, 1) - 1,
    ),
  };
}

export function isWorkbookAgentMediaCommandKind(
  kind: string,
): kind is WorkbookAgentMediaCommand["kind"] {
  return (
    kind === "upsertImage" ||
    kind === "deleteImage" ||
    kind === "upsertShape" ||
...
export function isWorkbookAgentMediaCommand(
  command: WorkbookAgentCommand,
): command is WorkbookAgentMediaCommand {
  return isWorkbookAgentMediaCommandKind(command.kind);
}

export function isWorkbookAgentMediaCommandValue(
  value: unknown,
): value is WorkbookAgentMediaCommand {
...
export function estimateWorkbookAgentMediaCommandAffectedCells(
  command: WorkbookAgentMediaCommand,
): number | null {
  switch (command.kind) {
    case "upsertImage":
...
export function deriveWorkbookAgentMediaCommandPreviewRanges(
  command: WorkbookAgentMediaCommand,
  ...
  switch (command.kind) {
    ...
    case "upsertShape":
      return [
        {
          ...targetRange(
            command.shape.sheetName,
            command.shape.address,
            command.shape.rows,
            command.shape.cols,
          ),
          role: "target",
...
export function applyWorkbookAgentMediaCommand(

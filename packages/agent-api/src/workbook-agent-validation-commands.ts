...
export type WorkbookAgentValidationCommand = Extract<
  WorkbookAgentCommand,
  { kind: "setDataValidation" } | { kind: "clearDataValidation" }
>;
...
function isWorkbookValidationListSource(
  value: unknown,
): value is WorkbookValidationListSourceSnapshot {
...
export function isWorkbookAgentValidationCommandKind(
  kind: string,
): kind is WorkbookAgentValidationCommand["kind"] {
  return kind === "setDataValidation" || kind === "clearDataValidation";
}

export function isWorkbookAgentValidationCommand(
  command: WorkbookAgentCommand,
): command is WorkbookAgentValidationCommand {
  return isWorkbookAgentValidationCommandKind(command.kind);
}

export function isWorkbookAgentValidationCommandValue(
  value: unknown,
): value is WorkbookAgentValidationCommand {
  if (!isRecord(value) || typeof value["kind"] !== "string") {
    return false;
  }
  switch (value["kind"]) {
...
export function isHighRiskWorkbookAgentValidationCommand(
  _command: WorkbookAgentValidationCommand,
): boolean {
  return false;
}

export function isWorkbookScopeValidationCommand(
  _command: WorkbookAgentValidationCommand,
): boolean {
  return false;
}

export function describeWorkbookAgentValidationCommand(
  command: WorkbookAgentValidationCommand,
): string {
  switch (command.kind) {
    case "setDataValidation":
...
export function estimateWorkbookAgentValidationCommandAffectedCells(
  command: WorkbookAgentValidationCommand,
): number {
  return countRangeCells(
    command.kind === "setDataValidation" ? command.validation.range : command.range,
  );
}

export function deriveWorkbookAgentValidationCommandPreviewRanges(
  command: WorkbookAgentValidationCommand,
...
export function applyWorkbookAgentValidationCommand(

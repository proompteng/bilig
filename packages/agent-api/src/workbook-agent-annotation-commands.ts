...
function isCommentEntry(
  value: unknown,
): value is WorkbookCommentThreadSnapshot["comments"][number] {
  return (
    isRecord(value) &&
...
function normalizeCommentThread(
  ...
  return {
    ...
    comments: thread.comments.map((entry) => ({
      ...
      ...(entry.authorDisplayName !== undefined
...
export function isWorkbookAgentAnnotationCommandKind(
  kind: string,
): kind is WorkbookAgentAnnotationCommand["kind"] {
  return (
    kind === "upsertCommentThread" ||
    kind === "deleteCommentThread" ||
    kind === "upsertNote" ||
...
export function isWorkbookAgentAnnotationCommand(
  command: WorkbookAgentCommand,
): command is WorkbookAgentAnnotationCommand {
  return isWorkbookAgentAnnotationCommandKind(command.kind);
}

export function isWorkbookAgentAnnotationCommandValue(
  value: unknown,
): value is WorkbookAgentAnnotationCommand {
...
export function isHighRiskWorkbookAgentAnnotationCommand(
  _command: WorkbookAgentAnnotationCommand,
): boolean {
  return false;
}

export function isWorkbookScopeAnnotationCommand(
  _command: WorkbookAgentAnnotationCommand,
): boolean {
  return false;
}

export function describeWorkbookAgentAnnotationCommand(
  command: WorkbookAgentAnnotationCommand,
): string {
  switch (command.kind) {
    case "upsertCommentThread":
...
export function estimateWorkbookAgentAnnotationCommandAffectedCells(
  _command: WorkbookAgentAnnotationCommand,
): number {
  return 1;
}

export function deriveWorkbookAgentAnnotationCommandPreviewRanges(
  command: WorkbookAgentAnnotationCommand,
...
export function applyWorkbookAgentAnnotationCommand(

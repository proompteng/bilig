...
export interface EngineRuntimeColumnStoreService {
  readonly getColumnSlice: (request: {
    sheetName: string;
    rowStart: number;
    rowEnd: number;
    col: number;
...
function getColumnSliceCacheKey(
  sheetName: string,
  col: number,
  rowStart: number,
  rowEnd: number,
...
export function createEngineRuntimeColumnStoreService(args: {
  ...
  const buildColumnSlice = (request: {
    sheetName: string;
    rowStart: number;
    rowEnd: number;
    col: number;
  ...
  const getColumnSlice = (request: {
    sheetName: string;
    rowStart: number;
    rowEnd: number;
    col: number;
  }): RuntimeColumnSlice => {
    const cacheKey = getColumnSliceCacheKey(
      request.sheetName,
      request.col,
      request.rowStart,
      request.rowEnd,
    ...
    normalizeLookupText(value) {
      return (
        value.stringId !== undefined && value.stringId !== 0
          ? normalizeStringId(value.stringId)
          : value.value
      ).toUpperCase();
    },
  };

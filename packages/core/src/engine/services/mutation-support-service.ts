...
export interface EngineMutationSupportService {
  ...
  readonly markFormulaChanged: (
    cellIndex: number,
    count: number,
  ) => Effect.Effect<number, EngineMutationError>;
  readonly markVolatileFormulasChanged: (
    count: number,
  ) => Effect.Effect<number, EngineMutationError>;
  readonly markSpillRootsChanged: (
    cellIndices: readonly number[],
    count: number,
  ) => Effect.Effect<number, EngineMutationError>;
  readonly markPivotRootsChanged: (
    cellIndices: readonly number[],
    count: number,
  ) => Effect.Effect<number, EngineMutationError>;
  readonly markExplicitChanged: (
    cellIndex: number,
    count: number,
  ) => Effect.Effect<number, EngineMutationError>;
  readonly composeMutationRoots: (
  ...
  readonly unionChangedSets: (
    ...sets: Array<readonly number[] | U32>
  ) => Effect.Effect<U32, EngineMutationError>;
  ...
  readonly ensureCellTracked: (
    sheetName: string,
    address: string,
  ) => Effect.Effect<number, EngineMutationError>;
  readonly ensureCellTrackedByCoords: (
    sheetId: number,
    row: number,
    col: number,
  ) => Effect.Effect<number, EngineMutationError>;
...
export function createEngineMutationSupportService(args: {
  ...
  const clearOwnedSpillNow = (cellIndex: number): number[] => {
    ...
    changedCellIndices.push(
      ...args.applyDerivedOp({ kind: "deleteSpillRange", sheetName, address }),
    );
  ...
  const materializeSpillNow = (
    ...
    for (let rowOffset = 0; rowOffset < arrayValue.rows; rowOffset += 1) {
      for (let colOffset = 0; colOffset < arrayValue.cols; colOffset += 1) {
        ...
        const targetIndex = ensureCellTrackedByCoordsNow(
          sheetId,
          owner.row + rowOffset,
          owner.col + colOffset,
        );
  ...
  const removeSheetRuntimeNow = (
    ...
    cellIndices.forEach((cellIndex) => {
      ...
      args.state.workbook.cellStore.flags[cellIndex] =
        (args.state.workbook.cellStore.flags[cellIndex] ?? 0) | CellFlags.PendingDelete;
    ...
    if (args.getSelectionState().sheetName === sheetName) {
      const nextSheet = [...args.state.workbook.sheetsByName.values()].toSorted(
        ...
        try: () => ensureCellTrackedByCoordsNow(sheetId, row, col),
          ...
          new EngineMutationError({
            message: mutationErrorMessage(
              `Failed to ensure cell coordinates ${sheetId}:${row}:${col}`,
              cause,
            ),
            cause,
          }),

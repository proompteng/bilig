...
export interface EngineEventService {
  ...
  readonly subscribeCells: (
    sheetName: string,
    addresses: readonly string[],
    listener: () => void,
  ) => Effect.Effect<() => void>;
  readonly subscribeBatches: (
    listener: (batch: EngineOpBatch) => void,
  ) => Effect.Effect<() => void>;
}

export function createEngineEventService(
  state: Pick<EngineRuntimeState, "events" | "workbook" | "batchListeners">,
): EngineEventService {
  return {
    subscribe(listener) {

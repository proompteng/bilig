...
async function applyAndExpectMetadataSemantics(
...
const undoCommandArbitrary: fc.Arbitrary<AsyncCommand<EngineMetadataModel, SpreadsheetEngine>> =
  fc.constant({
    check: (model) => model.applied.length > 0,
    run: async (model, real) => {
      if (!real.undo()) {
        throw new Error(
          `Undo command failed despite a non-empty metadata history\ncommandHistory=${model.history.join(" -> ")}`,
        );
      }
...
const redoCommandArbitrary: fc.Arbitrary<AsyncCommand<EngineMetadataModel, SpreadsheetEngine>> =
  fc.constant({
    check: (model) => model.undone.length > 0,
    run: async (model, real) => {
      if (!real.redo()) {
        throw new Error(
          `Redo command failed despite a non-empty metadata undo stack\ncommandHistory=${model.history.join(" -> ")}`,
        );
      }
  ...
  for (const seedName of metadataSeedNames) {
    it(`keeps metadata semantics aligned for ${seedName} seeded workbooks`, async () => {
      const seedSnapshot = await createEngineSeedSnapshot(
        seedName,
        `fuzz-core-metadata-${seedName}`,
      );
      const ran = await runModelProperty({
        suite: `core/metadata/${seedName}`,
        commands: fc.commands(
          [
            applySemanticCommandArbitrary(metadataSemanticActionArbitrary),

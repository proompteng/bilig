...
describe("engine metadata utils", () => {
  ...
  it("renames defined-name sheet references without touching structured refs", () => {
    expect(
      renameDefinedNameValueSheet(
        { kind: "cell-ref", sheetName: "Old Sheet", address: "A1" },
        "Old Sheet",
        "New Sheet",
      ),
    ).toEqual({
    ...
    expect(
      renameDefinedNameValueSheet(
        { kind: "formula", formula: "='Old Sheet'!A1+SUM('Old Sheet'!B:B)" },
        "Old Sheet",
        "New Sheet",
      ),
    ).toEqual({
    ...
    expect(
      renameDefinedNameValueSheet(
        { kind: "structured-ref", tableName: "Sales", columnName: "Amount" },
        "Old Sheet",
        "New Sheet",
      ),
    ).toEqual({
    ...
    expect(
      resolveMetadataReferencesInAst(formula, {
        ...
        resolveSpillReference: (sheetName, address) =>
          sheetName === "Sheet1" && address === "A1"
            ? { kind: "CellRef", sheetName: "Sheet1", ref: "C3" }
            : undefined,
      }),
    ).toEqual({
      fullyResolved: true,

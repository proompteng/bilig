    ...
    await runProperty({
      ...
      predicate: async (model) => {
        ...
        const shouldMaterialize = (raw: LiteralInput) =>
          raw !== null && !(model.skipFormulaStrings && typeof raw === 'string' && raw.startsWith('='))
        const loaded = loadLiteralSheetIntoEmptySheet(engine.workbook, engine.strings, sheetId, model.rows, shouldMaterialize)
...
const literalLoaderModelArbitrary: fc.Arbitrary<LiteralLoaderModel> = fc.record({
  rows: fc.array(
    fc.array(fc.oneof<LiteralInput>(fc.integer({ min: -20, max: 20 }), fc.boolean(), fc.string(), fc.constant(null)), {
      minLength: 0,
      maxLength: 5,
    }),
...
function countExpectedLoadedCells(rows: readonly (readonly LiteralInput[])[], shouldMaterialize: (raw: LiteralInput) => boolean): number {
  return rows.reduce(
    (count, row) => count + row.reduce((rowCount, raw) => rowCount + (raw !== null && shouldMaterialize(raw) ? 1 : 0), 0),
    0,
  )
}

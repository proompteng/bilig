...
describe("WorkbookMetadataService", () => {
  ...
  it("clones, normalizes, lists, and deletes chart, image, and shape metadata", () => {
    ...
    expect(Effect.runSync(service.listCharts()).map((record) => record.id)).toEqual([
      "chart-a",
      "chart-b",
    ]);
    expect(Effect.runSync(service.listImages()).map((record) => record.id)).toEqual([
      "image-a",
      "image-b",
    ]);
    expect(Effect.runSync(service.listShapes()).map((record) => record.id)).toEqual([
      "shape-a",
      "shape-c",
    ]);
  ...
  it("wraps metadata bucket failures with stable workbook metadata errors", () => {
    ...
    const cases = [
      ...
      },
      {
        ...
        run: (service: WorkbookMetadataService) =>
          service.setDefinedName("TaxRate", { kind: "scalar", value: 1 }),
      },
      {
        bucket: "definedNames",
    ...
    for (const testCase of cases) {
      ...
      const service = createWorkbookMetadataService(
        metadata as ReturnType<typeof createWorkbookMetadataRecord>,
      );

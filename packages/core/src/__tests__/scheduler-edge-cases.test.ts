...
describe("RecalcScheduler edge cases", () => {
  ...
  it("dedupes exact and sorted lookup columns independently across repeated traversals", () => {
    ...
    const getDependents = (entityId: number): Uint32Array =>
      graph.get(entityId) ?? new Uint32Array();
    const scheduler = new RecalcScheduler();

    const first = scheduler.collectDirty(
      Uint32Array.of(0),
      { getDependents },
      store,
    ...
    expect(
      Array.from(first.orderedFormulaCellIndices.subarray(0, first.orderedFormulaCount)),
    ).toEqual([1, 2]);
    expect(
      Array.from(second.orderedFormulaCellIndices.subarray(0, second.orderedFormulaCount)),
    ).toEqual([1, 2]);
  });
  ...
  it("resets lookup epochs at overflow and skips non-formula dependents", () => {
    ...
    const graph = new Map<number, Uint32Array>([
      [makeCellEntity(0), Uint32Array.of(makeCellEntity(1), makeCellEntity(2))],
    ]);
    const result = scheduler.collectDirty(
      Uint32Array.of(0),
  ...
  it("grows the entity queue when traversal adds more non-cell entities than the store size", () => {
    ...
    const graph = new Map<number, Uint32Array>([
      [
        makeCellEntity(0),
        Uint32Array.from(Array.from({ length: 200 }, (_, index) => makeRangeEntity(index))),
      ],
    ]);
  ...
  it("grows rank buffers when many dirty formulas require counting sort", () => {
    ...
    expect(Array.from(result.orderedFormulaCellIndices.subarray(125, 130))).toEqual([
      125, 126, 127, 128, 129,
    ]);
  });
});

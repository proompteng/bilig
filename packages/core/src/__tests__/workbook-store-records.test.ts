...
describe("workbook store records", () => {
  ...
  it("rejects invalid style and format identifiers", () => {
    expect(() => normalizeCellStyleRecord({ id: "   " })).toThrow(
      "Cell style id must be non-empty",
    );
    expect(() =>
      normalizeCellStyleRecord({
        id: "style-a",
    ...
    expect(() => normalizeCellNumberFormatRecord(format)).toThrow(
      "Cell number format id must be non-empty",
    );
  });

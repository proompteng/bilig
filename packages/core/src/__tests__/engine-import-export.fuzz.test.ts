  ...
  it("preserves CSV-representable single-sheet semantics across export and import", async () => {
    await runProperty({
      ...
      predicate: async (model) => {
        ...
        expect(projectCsvSemanticSheet(restored, bounds)).toEqual(
          projectCsvSemanticSheet(original, bounds),
        );
      },
    });
  });
          ...
          .chain(([baseCells, formulaTargets]) => {
            ...
            return formulaCellsArbitrary.map((formulaCells) => {
              const cells = Array.from({ length: rowCount }, () =>
                Array.from<CsvCellModel>({ length: colCount }),
              );
...
function csvFormulaCellModelArbitrary(
  referenceAddresses: readonly string[],
): fc.Arbitrary<CsvFormulaCellModel> {
  ...
  return fc.oneof(
    ...
    fc.tuple(referenceArbitrary, referenceArbitrary).map(([left, right]) => {
      ...
      const start = formatAddress(
        Math.min(leftAddress.row, rightAddress.row),
        Math.min(leftAddress.col, rightAddress.col),
      );
      const end = formatAddress(
        Math.max(leftAddress.row, rightAddress.row),
        Math.max(leftAddress.col, rightAddress.col),
      );
      return {
        kind: "formula",
...
async function createEngineFromCsvModel(
  workbookName: string,
  model: CsvSheetModel,
  ...
  for (let row = 0; row < model.rowCount; row += 1) {
    for (let col = 0; col < model.colCount; col += 1) {
      ...
      if (cell.kind === "value") {
        engine.setRangeValues({ sheetName, startAddress: address, endAddress: address }, [
          [cell.value],
        ]);
...
function projectCsvSemanticSheet(

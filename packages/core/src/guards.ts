...
export function isEngineReplicaSnapshot(value: unknown): value is EngineReplicaSnapshot {
  return (
    isRecord(value) && isRecord(value['replica']) && Array.isArray(value['entityVersions']) && Array.isArray(value['sheetDeleteVersions'])
  )
}
...
export function isCommitOp(value: unknown): value is CommitOp {
  ...
  switch (value['kind']) {
    ...
    case 'upsertSheet':
      return typeof value['name'] === 'string' && (value['order'] === undefined || typeof value['order'] === 'number')
    case 'renameSheet':
    ...
    case 'upsertCell':
      return (
        ...
        (isLiteralInput(value['value']) || typeof value['formula'] === 'string' || typeof value['format'] === 'string')
      )
    case 'deleteCell':

...
export function buildFormulaFamilyShapeKey(input: FormulaFamilyShapeKeyInput): string {
  return `${input.compiled.mode}|${input.compiled.deps.length}|${input.compiled.symbolicRanges.length}|${
    input.compiled.symbolicNames.length
  }|${input.compiled.symbolicTables.length}|${input.compiled.symbolicSpills.length}|${input.dependencyCount}|${
    input.rangeDependencyCount
  }|${input.directAggregateKind ?? ''}|${input.directLookupKind ?? ''}|${input.directScalarKind ?? ''}|${input.directCriteriaKind ?? ''}`
}

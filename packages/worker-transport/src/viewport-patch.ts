...
function encodeBorderSide(
  writer: BinaryWriter,
  side: NonNullable<NonNullable<CellStyleRecord["borders"]>["top"]> | undefined,
): void {
  if (!side) {
    writer.u8(0);
    return;
...
function decodeBorderSide(
  reader: BinaryReader,
): NonNullable<NonNullable<CellStyleRecord["borders"]>["top"]> | undefined {
  if (reader.u8() === 0) {
    return undefined;
  }
  return {
  ...
  if (style.alignment) {
    encodeOptionalTag(
      ...
      style.alignment.horizontal
        ? encodeHorizontalAlignment(style.alignment.horizontal)
        : undefined,
    );
    encodeOptionalTag(
      writer,
      style.alignment.vertical ? encodeVerticalAlignment(style.alignment.vertical) : undefined,
    );
